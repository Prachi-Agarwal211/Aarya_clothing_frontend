    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      ...
      {confirmFor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[#161616] border border-[#A8B4C8]/30 rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-amber-500/20 flex items-center justify-center">
                <Zap className="w-5 h-5 text-amber-400" />
              </div>
              <div>
                <h3 className="text-[#D4AF37] font-semibold">Force Create Order</h3>
                <p className="text-xs text-[#F5F0E8]/50">This will create a DB order immediately</p>
              </div>
            </div>
            <div className="p-3 bg-[#111111]/60 rounded-xl mb-4 space-y-1">
              <p className="text-xs text-[#F5F0E8]/50">Payment ID</p>
              <p className="font-mono text-sm text-[#D4AF37] break-all">{confirmFor.payment_id}</p>
              <p className="text-xs text-[#F5F0E8]/50 mt-2">Amount</p>
              <p className="text-green-400 font-semibold">{formatCurrency(confirmFor.amount)}</p>
              {confirmFor.email && <>
                <p className="text-xs text-[#F5F0E8]/50 mt-2">Customer</p>
                <p className="text-sm text-[#F5F0E8]/80">{confirmFor.email}</p>
              </>}
            </div>
            <p className="text-xs text-[#F5F0E8]/50 mb-4">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-2 border border-[#A8B4C8]/20 text-[#F5F0E8]/60 rounded-xl hover:bg-[#A8B4C8]/10 transition-colors text-sm"
              >
                Cancel
              </button>
              <button
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        {data && (
          <button
            onClick={downloadCsv}
            className="flex items-center gap-2 px-4 py-2 bg-[#1E3A5F]/30 border border-[#A8B4C8]/30 text-[#D4AF37] rounded-xl hover:bg-[#1E3A5F]/50 transition-colors text-sm"
          >
      ...
      {/* Controls */}
      <div className="p-5 bg-[#111111]/60 border border-[#A8B4C8]/15 rounded-2xl flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          <Clock className="w-4 h-4 text-[#A8B4C8]" />
          <span className="text-sm text-[#F5F0E8]/70">Look back:</span>
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg text-sm transition-colors ${
                hours === h
                  ...
                  : 'bg-[#1E3A5F]/20 text-[#F5F0E8]/60 hover:bg-[#1E3A5F]/40'
              }`}
            >
              {h === 168 ? '7d' : `${h}h`}
            </button>
        ...
        <button
          onClick={fetchReport}
          disabled={loading}
          className="flex items-center gap-2 px-5 py-2 bg-gradient-to-r from-[#1E3A5F] to-[#A8B4C8] text-white font-medium rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50 text-sm ml-auto"
        >
      ...
      {data && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { label: 'Payments Fetched', value: data.total_payments_fetched, color: 'text-[#F5F0E8]' },
              ...
              <div key={label} className="p-4 bg-[#111111]/60 border border-[#A8B4C8]/15 rounded-xl">
                <p className="text-xs text-[#F5F0E8]/50 mb-1">{label}</p>
          ...
          {/* Tabs */}
          <div className="flex gap-2 border-b border-[#A8B4C8]/15">
            {[
              ...
              <button
                ...
                className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
                  tab === key
                    ? 'border-[#A8B4C8] text-[#D4AF37]'
                    : 'border-transparent text-[#F5F0E8]/50 hover:text-[#F5F0E8]'
                }`}
              >
                {label}
              </button>
          ...
          {tab === 'missing' && (
            <div className="bg-[#111111]/60 border border-[#A8B4C8]/15 rounded-2xl overflow-hidden">
              {data.missing_orders.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16">
                  <CheckCircle className="w-12 h-12 text-green-400 mb-3" />
                  <p className="text-[#D4AF37] font-semibold">All payments have orders — no recovery needed!</p>
                </div>
              ) : (
                    ...
                    <thead>
                      <tr className="border-b border-[#A8B4C8]/15">
                        {['Payment ID', 'Rzp Order ID', 'Amount', 'Customer', 'Method', 'Date', 'Action'].map(h => (
                          <th key={h} className="px-4 py-3 text-left text-xs text-[#F5F0E8]/50 uppercase tracking-wider font-medium whitespace-nowrap">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#A8B4C8]/10">
                      {data.missing_orders.map(p => {
                        const result = createResults[p.payment_id];
                        const isCreating = creatingFor === p.payment_id;
                        return (
                          <React.Fragment key={p.payment_id}>
                            <tr className={`hover:bg-[#A8B4C8]/5 transition-colors ${result?.success ? 'opacity-50' : ''}`}>
                              <td className="px-4 py-3 font-mono text-xs text-[#D4AF37] whitespace-nowrap">{p.payment_id}</td>
                              <td className="px-4 py-3 font-mono text-xs text-[#F5F0E8]/50 whitespace-nowrap">{p.razorpay_order_id || '—'}</td>
                              <td className="px-4 py-3 text-green-400 font-semibold whitespace-nowrap">{formatCurrency(p.amount)}</td>
                              <td className="px-4 py-3">
                                <div className="space-y-0.5">
                                  {p.email && (
                                    <div className="flex items-center gap-1 text-[#F5F0E8]/70">
                                      <Mail className="w-3 h-3 shrink-0" />
                                      <span className="text-xs truncate max-w-[150px]">{p.email}</span>
                                    </div>
                                  )}
                                  {p.contact && (
                                    <div className="flex items-center gap-1 text-[#F5F0E8]/50">
                                      <Phone className="w-3 h-3 shrink-0" />
                                      <span className="text-xs">{p.contact}</span>
                                    </div>
                                  )}
                                  {!p.email && !p.contact && <span className="text-xs text-[#F5F0E8]/30">No contact info</span>}
                                </div>
                              </td>
                              <td className="px-4 py-3 text-xs text-[#F5F0E8]/60 capitalize whitespace-nowrap">{p.method || '—'}</td>
                              <td className="px-4 py-3 text-xs text-[#F5F0E8]/50 whitespace-nowrap">{formatTimestamp(p.created_at)}</td>
                              ...
                              <tr>
                                <td colSpan={7} className="px-4 py-2 bg-green-500/5">
                                  <div className="flex items-center gap-2 text-xs text-green-400">
                                    ...
                                    <a href={`/admin/orders`} className="ml-auto flex items-center gap-1 text-[#A8B4C8] hover:text-[#D4AF37]">
          ...
          {tab === 'matched' && (
            <div className="bg-[#111111]/60 border border-[#A8B4C8]/15 rounded-2xl overflow-hidden">
              {data.matched.length === 0 ? (
                <div className="py-12 text-center text-[#F5F0E8]/50">No matched payments in this period.</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-[#A8B4C8]/15">
                        {['Payment ID', 'Order ID', 'Amount', 'Customer Email', 'Phone', 'Date', 'Status'].map(h => (
                          <th key={h} className="px-4 py-3 text-left text-xs text-[#F5F0E8]/50 uppercase tracking-wider font-medium">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#A8B4C8]/10">
                      {data.matched.map(p => (
                        <tr key={p.payment_id} className="hover:bg-[#A8B4C8]/5 transition-colors">
                          <td className="px-4 py-3 font-mono text-xs text-[#D4AF37]">{p.payment_id}</td>
                          <td className="px-4 py-3 font-mono text-xs text-[#F5F0E8]/70">{p.order_id || '—'}</td>
                          <td className="px-4 py-3 text-green-400 font-semibold">{formatCurrency(p.amount)}</td>
                          <td className="px-4 py-3 text-xs text-[#F5F0E8]/70 truncate max-w-[160px]">{p.email || '—'}</td>
                          <td className="px-4 py-3 text-xs text-[#F5F0E8]/60">{p.contact || '—'}</td>
                          <td className="px-4 py-3 text-xs text-[#F5F0E8]/50">{formatTimestamp(p.created_at)}</td>
                          <td className="px-4 py-3">
      ...
      {!data && !loading && !error && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          <IndianRupee className="w-14 h-14 text-[#A8B4C8]/30 mb-4" />
          <p className="text-[#F5F0E8]/50 mb-2">Click &quot;Run Recovery Check&quot; to fetch Razorpay payments</p>
          <p className="text-xs text-[#F5F0E8]/30">Compares captured payments against orders in the database</p>
        </div>
      )}
    </div>
  );
