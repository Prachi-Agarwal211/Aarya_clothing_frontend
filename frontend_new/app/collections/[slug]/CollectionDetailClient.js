...
export default function CollectionDetailClient({ initialCollection, initialProducts, slug }) {
  ...
  useEffect(() => {
    if (category) {
      // Only fetch if we don't have initial products, or if page changes
      if (initialProducts.length === 0 || filters.page > 1) {
        fetchProducts();
      } else {
  ...
  }, [category, filters.page, fetchProducts, initialProducts]);
