      ...
      <div className="relative z-10 page-wrapper">
        ...
        <div className="page-content">
          <div className="container mx-auto px-4 sm:px-6 md:px-8 header-spacing">
            ...
            {cart?.subtotal > 0 && cart?.subtotal < 5000 && (
              <div className="mb-8 p-4 bg-gradient-to-r from-[#7A2F57]/20 to-transparent border-l-4 border-[#B76E79] rounded-r-2xl">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-sm font-medium text-[#F2C29A]">
                    You are ₹{(5000 - cart.subtotal).toLocaleString()} away from <span className="underline decoration-[#F2C29A] underline-offset-4">FREE SHIPPING</span>
                  </p>
                  <span className="text-[10px] text-[#EAE0D5]/40 uppercase tracking-widest font-cinzel">Special Offer</span>
                </div>
                <div className="w-full h-1.5 bg-[#B76E79]/10 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-[#7A2F57] to-[#F2C29A] transition-all duration-1000 ease-out"
                    style={{ width: `${Math.min((cart.subtotal / 5000) * 100, 100)}%` }}
                  />
                </div>
              </div>
            )}
