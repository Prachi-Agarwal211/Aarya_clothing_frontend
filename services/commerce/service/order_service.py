...
class OrderService:
    ...
    def create_order(
        ...
        # Clear cart — must happen after DB commit (Redis op).
        # Failures here are non-fatal: the order is already persisted.
        # The reconciler background task will clean stale cart state.
        try:
            self.cart_service.clear_cart(user_id, release_reservations=False)
        except Exception as e:
            logger.warning(
                "Failed to clear cart for user %s after order %s creation: %s. "
                "Cart will be reconciled by background task.",
                user_id, order.id, e
            )
    ...
    async def initiate_payment(
        ...
        Args:
            order_id: Order ID
            payment_method: Payment method (razorpay, cod, etc.)
    ...
    async def process_refund(
        ...
        if not order.transaction_id:
            logger.warning(f"Order {order_id} has no transaction ID - may be COD")
            return {
                "refund_id": None,
                "order_id": order_id,
    ...
    def _send_order_cancelled_email(self, order: Order, reason: Optional[str] = None) -> bool:
        ...
        refund_info = "Refund will be processed within 5-7 business days." if order.payment_method != "cod" else "No refund required (Cash on Delivery order)."
