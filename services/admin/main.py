...
import asyncio
from fastapi import FastAPI, Depends, HTTPException, Query, Request, WebSocket, WebSocketDisconnect, UploadFile, File, status
import httpx
...
from core.validation import (
    validate_order_status, validate_user_role, validate_search_query,
    validate_pagination, sanitize_order_updates
)
from core.exception_handler import (
    setup_exception_handlers, log_security_event, log_business_event,
    AdminServiceException, ValidationException, DatabaseException
)
...
from schemas.admin import (
    ...
    UserListItem, UserStatusUpdate, BulkUserStatusUpdate,
    ...
    LandingProductCreate, LandingProductUpdate,
    ProductCreate, ProductUpdate,
    CategoryCreate, CategoryUpdate,
    BulkPriceUpdate, BulkStatusUpdate, BulkCollectionAssign,
...
async def get_dashboard_overview(db: Session = Depends(get_db), user: dict = Depends(require_admin)):
    ...
    result = DashboardOverview(
        total_revenue=float(stats[0]), total_orders=stats[1],
        total_customers=stats[2], total_products=stats[3],
        pending_orders=stats[4], today_revenue=float(stats[5]),
...
async def admin_list_inventory(
    ...
    total = db.execute(text("SELECT COUNT(*) FROM inventory")).scalar() or 0
...
async def admin_out_of_stock(db: Session = Depends(get_db), user: dict = Depends(require_admin)):
    rows = db.execute(text(
        "SELECT i.*, p.name as product_name FROM inventory i JOIN products p ON p.id = i.product_id "
...
async def admin_inventory_movements(
    ...
    if product_id:
        ...
        """), {"pid": product_id, "lim": limit, "off": offset}).fetchall()
        ...
        total = db.execute(text("SELECT COUNT(*) FROM inventory_movements")).scalar() or 0
...
async def admin_create_inventory(
    ...
    product = db.execute(text("SELECT id FROM products WHERE id = :pid"), {"pid": product_id}).fetchone()
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")

    # Check for duplicate SKU
    existing = db.execute(text("SELECT id FROM inventory WHERE sku = :sku"), {"sku": sku}).fetchone()
    if existing:
        raise HTTPException(status_code=409, detail=f"Inventory with SKU '{sku}' already exists")
...
async def admin_update_inventory(
    ...
    inv = db.execute(text("SELECT id FROM inventory WHERE id = :id"), {"id": inventory_id}).fetchone()
    if not inv:
        raise HTTPException(status_code=404, detail="Inventory record not found")

    ALLOWED = {"quantity", "low_stock_threshold", "size", "color"}
    sets, params = ["updated_at = :now"], {"id": inventory_id, "now": datetime.now(timezone.utc)}
...
async def admin_adjust_inventory(
    ...
    inv = db.execute(
        text("SELECT id, product_id, quantity FROM inventory WHERE sku = :sku"), {"sku": sku}
    ).fetchone()
    if not inv:
    ...
    try:
        ...
        ), {"iid": inv[0], "pid": inv[1], "adj": adjustment, "reason": reason, "notes": notes, "now": datetime.now(timezone.utc)})
    except Exception:
        pass  # movements table may not exist yet
    db.commit()
    redis_client.invalidate_pattern("products:*")
    return {"sku": sku, "previous_quantity": inv[2], "new_quantity": new_qty, "adjustment": adjustment}
...
async def get_staff_dashboard(db: Session = Depends(get_db), user: dict = Depends(require_staff)):
    ...
    pending = db.execute(text("SELECT COUNT(*) FROM orders WHERE status = 'confirmed'")).scalar() or 0
...
async def add_stock(data: AddStockRequest, db: Session = Depends(get_db), user: dict = Depends(require_staff)):
    ...
    if existing:
        ...
        db.execute(text("UPDATE inventory SET quantity = :q, updated_at = :now WHERE id = :id"), {"q": new_qty, "now": datetime.now(timezone.utc), "id": existing[0]})
        inv_id = existing[0]
    else:
        result = db.execute(text(
            "INSERT INTO inventory (product_id, sku, quantity, cost_price, created_at, updated_at) "
            "VALUES (:pid, :sku, :qty, :cp, :now, :now) RETURNING id"
        ), {"pid": data.product_id, "sku": data.sku, "qty": data.quantity, "cp": data.cost_price, "now": datetime.now(timezone.utc)})
        inv_id = result.scalar()
    # Record movement
    db.execute(text(
...
async def adjust_stock(data: AdjustStockRequest, db: Session = Depends(get_db), user: dict = Depends(require_staff)):
    ...
    db.execute(text("UPDATE inventory SET quantity = :q, updated_at = :now WHERE id = :id"), {"q": new_qty, "now": datetime.now(timezone.utc), "id": data.inventory_id})
    db.execute(text(
    ...
    for item in data.updates:
        ...
        if "quantity" in item:
            ...
            if not isinstance(qty, int) or qty < 0 or qty > 100_000:
                raise HTTPException(status_code=400, detail=f"quantity must be an integer 0-100000, got {qty!r}")
        if "low_stock_threshold" in item:
            thresh = item["low_stock_threshold"]
            if not isinstance(thresh, int) or thresh < 0 or thresh > 10_000:
                raise HTTPException(status_code=400, detail=f"low_stock_threshold must be an integer 0-10000, got {thresh!r}")
        ...
        if "quantity" in item and "low_stock_threshold" in item:
            ...
            ), {"qty": item["quantity"], "thresh": item["low_stock_threshold"],
                "now": datetime.now(timezone.utc), "id": inv_id})
            updated += 1
        elif "quantity" in item:
            db.execute(text(
                "UPDATE inventory SET quantity = :qty, updated_at = :now WHERE id = :id"
            ), {"qty": item["quantity"], "now": datetime.now(timezone.utc), "id": inv_id})
            updated += 1
        elif "low_stock_threshold" in item:
            db.execute(text(
                "UPDATE inventory SET low_stock_threshold = :thresh, updated_at = :now WHERE id = :id"
            ), {"thresh": item["low_stock_threshold"], "now": datetime.now(timezone.utc), "id": inv_id})
            updated += 1
...
async def get_variants(product_id: int, db: Session = Depends(get_db), user: dict = Depends(require_staff)):
    rows = db.execute(
        text("SELECT * FROM inventory WHERE product_id = :pid ORDER BY size, color, sku"),
        {"pid": product_id}
    ).fetchall()
...
async def create_variant(product_id: int, data: VariantCreate, db: Session = Depends(get_db), user: dict = Depends(require_staff)):
    ...
    ), {"pid": product_id, "sku": data.sku, "size": data.size, "color": data.color, "qty": data.quantity, "now": datetime.now(timezone.utc)})
    db.commit()
...
async def update_variant(variant_id: int, data: VariantUpdate, db: Session = Depends(get_db), user: dict = Depends(require_staff)):
    sets, params = ["updated_at = :now"], {"id": variant_id, "now": datetime.now(timezone.utc)}
    ...
    if data.quantity is not None: sets.append("quantity = :qty"); params["qty"] = data.quantity
    db.execute(text(f"UPDATE inventory SET {', '.join(sets)} WHERE id = :id"), params)
    db.commit()
    redis_client.invalidate_pattern("products:*")
...
async def staff_confirmed_orders(db: Session = Depends(get_db), user: dict = Depends(require_staff)):
...
async def ship_order(order_id: int, data: OrderShipRequest, db: Session = Depends(get_db), user: dict = Depends(require_staff)):
    ...
    if not data.tracking_number or not data.tracking_number.strip():
        raise HTTPException(status_code=400, detail="POD number is required when shipping an order")
    order = db.execute(text("SELECT id, status FROM orders WHERE id = :id"), {"id": order_id}).fetchone()
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
    if order[1] != "confirmed":
        raise HTTPException(status_code=400, detail=f"Only confirmed orders can be shipped (current status: {order[1]})")
    now = datetime.now(timezone.utc)
    db.execute(text(
    ...
    return {"message": "Order shipped successfully", "order_id": order_id, "pod_number": data.tracking_number.strip()}


@app.put("/api/v1/staff/orders/{order_id}/deliver", tags=["Staff Orders"])
async def deliver_order(order_id: int, db: Session = Depends(get_db), user: dict = Depends(require_staff)):
    """Mark a shipped order as delivered."""
    order = db.execute(text("SELECT id, status FROM orders WHERE id = :id"), {"id": order_id}).fetchone()
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
    if order[1] != "shipped":
        raise HTTPException(status_code=400, detail=f"Only shipped orders can be marked delivered (current status: {order[1]})")
    now = datetime.now(timezone.utc)
    db.execute(text(
...
async def processed_orders_report(db: Session = Depends(get_db), user: dict = Depends(require_staff)):
    today = datetime.now(timezone.utc).date()
    shipped_today = db.execute(text("SELECT COUNT(*) FROM orders WHERE status = 'shipped' AND DATE(shipped_at) = :d"), {"d": today}).scalar() or 0
    delivered_today = db.execute(text("SELECT COUNT(*) FROM orders WHERE status = 'delivered' AND DATE(delivered_at) = :d"), {"d": today}).scalar() or 0
    confirmed_pending = db.execute(text("SELECT COUNT(*) FROM orders WHERE status = 'confirmed'")).scalar() or 0
    return {"today_shipped": shipped_today, "today_delivered": delivered_today, "confirmed_awaiting_shipment": confirmed_pending}
...
async def send_chat_message(room_id: int, data: ChatMessageCreate, db: Session = Depends(get_db), user: dict = Depends(require_staff)):
    ...
    db.execute(text("UPDATE chat_rooms SET updated_at = :now WHERE id = :rid"), {"now": datetime.now(timezone.utc), "rid": room_id})
    db.commit()
...
async def _save_chat_message_async(room_id: int, user_id: int, sender_type: str, msg_text: str):
    ...
    try:
        ...
        db_session.execute(text(
            "UPDATE chat_rooms SET updated_at = :now WHERE id = :rid"
...
async def websocket_chat(ws: WebSocket, room_id: int, token: str = Query(None)):
    ...
    Connect with: ws://host/api/v1/chat/ws/{room_id}?token=<jwt>
    Send JSON: {"message": "text"}
    Receive JSON: {"sender_id": int, "sender_type": str, "message": str, "timestamp": str}
...
async def get_site_config(db: Session = Depends(get_db), user: dict = Depends(require_admin)):
    rows = db.execute(text("SELECT key, value, description, updated_at FROM site_config")).fetchall()
    return {"config": {r[0]: {"value": r[1], "description": r[2], "updated_at": str(r[3])} for r in rows}}

@app.put("/api/v1/admin/site/config", tags=["Site Config"])
async def update_site_config(data: dict, db: Session = Depends(get_db), user: dict = Depends(require_admin)):
...
async def list_all_orders(
    ...
    if status:
        ...
        params["status"] = status
    if search:
        where_clauses.append("(CAST(o.id AS TEXT) ILIKE :search OR u.email ILIKE :search OR u.username ILIKE :search OR COALESCE(o.tracking_number, '') ILIKE :search)")
    ...
    total = db.execute(text(
        f"SELECT COUNT(*) FROM orders o {where_clause}"
    ), params).scalar() or 0

    orders = [{
        "id": r[0], "user_id": r[1], "subtotal": float(r[2] or 0),
        "discount_applied": float(r[3] or 0), "shipping_cost": float(r[4] or 0),
        "total_amount": float(r[5] or 0), "payment_method": r[6], "status": r[7],
        "tracking_number": r[8], "order_notes": r[9],
        "created_at": str(r[10]), "updated_at": str(r[11]),
        "customer_email": r[12], "customer_name": r[13],
    } for r in rows]
...
async def update_order_status(
    ...
    if new_status == "shipped" and not pod_number:
        raise HTTPException(status_code=400, detail="POD number is required when marking order as shipped")
    ...
    result = db.execute(
        text(f"UPDATE orders SET status = :status, updated_at = :now{extra_sets} WHERE id = :id RETURNING id"),
        params
    )
    db.commit()
    ...
    default_notes = {
        "shipped": f"Order shipped — POD number: {pod_number}" if pod_number else "Order shipped",
    ...
    log_business_event("order_status_updated", {
        "order_id": order_id,
...
async def bulk_update_order_status(
    ...
    updated = db.execute(text("""
        ...
        RETURNING id
    """), {"status": data.status, "now": now, "order_ids": data.order_ids}).fetchall()
...
async def export_orders_excel(
    ...
    headers = [
        "Order ID", "Order #", "Customer Email", "Customer Name",
        "Total (₹)", "Subtotal (₹)", "Discount (₹)", "Shipping (₹)",
        "Payment Method", "Status", "POD / Tracking No.",
        "Shipping Address", "Order Date", "Shipped Date", "Delivered Date",
        "Cancellation Reason"
    ]

    header_fill = PatternFill(start_color="180F14", end_color="180F14", fill_type="solid")
    ...
    for row_idx, r in enumerate(rows, 2):
        ...
        ws.cell(row=row_idx, column=13, value=str(r[11])[:19] if r[11] else "")  # created_at
        ws.cell(row=row_idx, column=14, value=str(r[12])[:19] if r[12] else "")  # shipped_at
        ws.cell(row=row_idx, column=15, value=str(r[13])[:19] if r[13] else "")  # delivered_at
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        try:
            ...
            if str(order[1]) not in ("confirmed",):
                errors.append(f"Order #{order_id}: status is '{order[1]}', can only ship confirmed orders")
                skipped += 1
                continue
...
async def download_pod_template(
    ...
    headers = ["Order ID", "Order #", "Customer Email", "Customer Name",
               "Total (₹)", "Order Date", "Shipping Address", "POD / Tracking No."]
...
async def ai_customer_chat(
    ...
    language    = body.get("language", "auto")  # 'auto' | 'en' | 'hi'
    user_id     = None  # guest or extract from optional token
    ...
    # Optional: extract user_id from auth token if present
    try:
    ...
    return customer_chat(db, message, session_id, user_id, language=language)
...
async def get_ai_sessions(
    ...
    return {
        "sessions": [
            {
                "session_id": r[0], "role": r[1], "user_id": r[2], "email": r[3],
                "messages": r[4], "tokens_in": r[5], "tokens_out": r[6],
                "cost_usd": float(r[7] or 0),
                "created_at": str(r[8]), "last_activity": str(r[9])
            }
            for r in sessions
        ]
    }
...
async def execute_ai_action(
    ...
    allowed_actions = {"ship_order", "update_product_price", "bulk_update_category_prices",
                       "adjust_stock", "create_product_draft"}
    if action_type not in allowed_actions:
        raise HTTPException(status_code=400, detail=f"Action '{action_type}' is not permitted.")

    result = execute_confirmed_action(db, action_type, params, admin_user_id)
    if not result.get("success"):
        raise HTTPException(status_code=422, detail=result.get("message", "Action failed."))
    return result
...
async def get_embedding_status(
    ...
    with_emb = db.execute(sa_text("SELECT COUNT(*) FROM products WHERE embedding IS NOT NULL")).scalar() or 0
...
@app.get("/api/v1/ai/admin/monitoring", tags=["AI"])
async def get_ai_monitoring(
    ...
    return {
        ...
        "daily": [
            {"date": str(r[0]), "tokens_in": r[1] or 0, "tokens_out": r[2] or 0,
             "cost_usd": float(r[3] or 0), "messages": r[4], "model": r[5]}
            for r in daily
        ],
        "by_model": [
            {"model": r[0], "sessions": r[1], "responses": r[2],
             "tokens_in": r[3] or 0, "tokens_out": r[4] or 0,
             "cost_usd": float(r[5] or 0)}
            for r in by_model
        ],
        "top_users": [
            {"email": r[0], "username": r[1], "role": r[2],
             "sessions": r[3], "cost_usd": float(r[4] or 0)}
            for r in top_users
        ],
    }
...
async def get_session_messages(
    ...
    return {
        "session": {
            "session_id": session[0], "role": session[1],
            "user_id": session[2], "email": session[3], "username": session[4],
            "message_count": session[5], "tokens_in": session[6],
            "tokens_out": session[7], "cost_usd": float(session[8] or 0),
            "created_at": str(session[9]), "last_activity": str(session[10]),
        },
        "messages": [
            {
                "id": r[0], "role": r[1], "content": r[2],
                "tokens_in": r[3], "tokens_out": r[4],
                "cost_usd": float(r[5] or 0), "model": r[6],
                "tool_calls": r[7], "tool_results": r[8],
                "has_images": bool(r[9]), "created_at": str(r[10]),
            }
            for r in messages
        ],
    }
...
async def export_ai_sessions_csv(
    ...
    import csv, io
    role_clause = f"AND s.role = '{role}'" if role and role in ("customer", "admin") else ""
    rows = db.execute(text(f"""
    ...
    writer.writerow(["session_id", "role", "email", "username", "messages",
                     "tokens_in", "tokens_out", "cost_usd", "created_at", "last_activity"])
    for r in rows:
        writer.writerow([r[0], r[1], r[2] or "", r[3] or "", r[4],
                         r[5], r[6], f"{float(r[7] or 0):.8f}", str(r[8]), str(r[9])])

    content = buf.getvalue().encode("utf-8")
    return Response(
        content=content,
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment; filename=ai_sessions_{days}d.csv"},
    )


# ── AI Settings (CRUD) ────────────────────────────────────────────────────────

@app.get("/api/v1/admin/ai-settings", tags=["AI Settings"])
...
@app.post("/api/v1/admin/ai-settings/test-key", tags=["AI Settings"])
async def test_gemini_api_key(
    data: dict,
    db: Session = Depends(get_db),
    current_user: dict = Depends(require_admin)
):
    """Test a Gemini API key by making a minimal ping request."""
    ...
    try:
        ...
        return {"valid": True, "response": resp.text.strip()}
    except Exception as e:
        return {"valid": False, "error": str(e)}


@app.put("/api/v1/admin/ai-settings/bulk", tags=["AI Settings"])
async def bulk_update_ai_settings(
    data: dict,
    db: Session = Depends(get_db),
    current_user: dict = Depends(require_admin)
):
    """Bulk update multiple AI settings at once. Must be defined before /{key}."""
    settings_data: dict = data.get("settings", {})
    user_id = current_user.get("user_id")
    now = datetime.now(timezone.utc)
    for k, value in settings_data.items():
        existing = db.execute(text("SELECT id FROM ai_settings WHERE key = :k"), {"k": k}).fetchone()
        if existing:
            db.execute(text("""
                UPDATE ai_settings SET value = :v, updated_at = :now, updated_by = :uid WHERE key = :k
            """), {"v": str(value), "now": now, "uid": user_id, "k": k})
        else:
            db.execute(text("""
                INSERT INTO ai_settings (key, value, updated_by, updated_at) VALUES (:k, :v, :uid, :now)
            """), {"k": k, "v": str(value), "uid": user_id, "now": now})
        if k == "GEMINI_API_KEY" and value:
            os.environ["GEMINI_API_KEY"] = str(value)
    db.commit()
    return {"success": True, "updated": list(settings_data.keys())}


@app.put("/api/v1/admin/ai-settings/{key}", tags=["AI Settings"])
async def update_ai_setting(
    ...
    """Update a single AI setting value."""
    value = data.get("value", "")
    user_id = current_user.get("user_id")

    existing = db.execute(text("SELECT id FROM ai_settings WHERE key = :k"), {"k": key}).fetchone()
    if existing:
        db.execute(text("""
            UPDATE ai_settings SET value = :v, updated_at = :now, updated_by = :uid
            WHERE key = :k
        """), {"v": value, "now": datetime.now(timezone.utc), "uid": user_id, "k": key})
    else:
        db.execute(text("""
            INSERT INTO ai_settings (key, value, updated_by, updated_at)
            VALUES (:k, :v, :uid, :now)
        """), {"k": key, "v": value, "uid": user_id, "now": datetime.now(timezone.utc)})
    db.commit()
...
async def update_landing_config(section: str, data: LandingConfigUpdate, db: Session = Depends(get_db), user: dict = Depends(require_admin)):
    ...
    if existing:
        updates = {"config": config_json, "updated_by": user_id, "now": datetime.now(timezone.utc), "s": section}
...
async def add_landing_image(data: LandingImageCreate, db: Session = Depends(get_db), user: dict = Depends(require_admin)):
    ...
    ), {"s": data.section, "url": data.image_url, "title": data.title, "sub": data.subtitle, "link": data.link_url, "order": data.display_order, "variant": data.device_variant})
    db.commit()
    redis_client.invalidate_pattern("public:landing:*")  # Invalidate cache so changes reflect immediately
...
async def upload_landing_image(
    ...
    result = db.execute(text(
        "INSERT INTO landing_images (section, image_url, title, subtitle, link_url, display_order, device_variant) "
        "VALUES (:s, :url, :title, :sub, :link, :order, :variant) RETURNING id"
    ), {"s": section, "url": image_url, "title": title, "sub": subtitle, "link": link_url, "order": display_order, "variant": device_variant})
    db.commit()
...
async def update_landing_image(
    ...
    if subtitle is not None: sets.append("subtitle = :subtitle"); params["subtitle"] = subtitle
    if link_url is not None: sets.append("link_url = :link_url"); params["link_url"] = link_url
    if display_order is not None: sets.append("display_order = :display_order"); params["display_order"] = display_order
    if is_active is not None: sets.append("is_active = :is_active"); params["is_active"] = is_active
    if not sets:
        return {"message": "Nothing to update"}
    db.execute(text(f"UPDATE landing_images SET {', '.join(sets)} WHERE id = :id"), params)
    db.commit()
    redis_client.invalidate_pattern("public:landing:*")  # Invalidate cache so changes reflect immediately
...
async def reorder_landing_images(
    ...
    for idx, image_id in enumerate(ordered_ids):
        db.execute(text("UPDATE landing_images SET display_order = :order WHERE id = :id AND section = :section"),
...
async def get_landing_products(
    ...
    for r in rows:
        products.append({
            "id": r[0], "section": r[1], "product_id": r[2],
...
async def add_landing_product(
    ...
    product_exists = db.execute(text("SELECT id FROM products WHERE id = :pid AND is_active = true"), {"pid": data.product_id}).fetchone()
    ...
    result = db.execute(text(
        ...
        "RETURNING id"
    ), {"s": data.section, "pid": data.product_id, "order": data.display_order, "active": data.is_active})
    db.commit()
    redis_client.invalidate_pattern("public:landing:*")
    return {"message": "Product added to landing section", "landing_product_id": result.scalar()}


@app.patch("/api/v1/admin/landing/products/{landing_product_id}", tags=["Landing Config"])
async def update_landing_product(
    ...
    if data.display_order is not None:
        sets.append("display_order = :order"); params["order"] = data.display_order
    if data.is_active is not None:
        sets.append("is_active = :active"); params["active"] = data.is_active
    if not sets:
        return {"message": "Nothing to update"}
    db.execute(text(f"UPDATE landing_products SET {', '.join(sets)} WHERE id = :id"), params)
    db.commit()
    redis_client.invalidate_pattern("public:landing:*")
    return {"message": "Landing product updated"}


@app.delete("/api/v1/admin/landing/products/{landing_product_id}", tags=["Landing Config"])
async def delete_landing_product(
    ...
    db.execute(text("DELETE FROM landing_products WHERE id = :id"), {"id": landing_product_id})
    db.commit()
    redis_client.invalidate_pattern("public:landing:*")
...
async def get_public_landing_images(
    ...
    if section:
        ...
        section_active = db.execute(text(
            "SELECT is_active FROM landing_config WHERE section = :s"
        ), {"s": section}).fetchone()
...
async def get_public_landing_all(db: Session = Depends(get_db)):
    ...
    logger.info(f"Landing API - sections: {len(sections)}, image_rows: {len(image_rows)}, has_data: {has_database_data}")
    if sections:
    ...
    try:
        na_limit = (sections.get("newArrivals", {}).get("config") or {}).get("max_display", 8)
        ...
        result["newArrivals"]["products"] = [
            {
                ...
                "is_new_arrival": r[6], "is_featured": r[7],
                "collection_name": r[9], "collection_slug": r[10],
    ...
    try:
        coll_limit = (sections.get("collections", {}).get("config") or {}).get("max_display", 6)
    ...
    if "about" in sections and sections["about"].get("config"):
        ...
        result["about"]["story"] = about_config.get("description", about_config.get("story", ""))
    ...
    if about_images:
        result["about"]["images"] = [
            _get_r2_public_url(r[2])
            for r in sorted(about_images, key=lambda x: x[6])
        ]
...
async def get_site_config(db: Session = Depends(get_db)):
    ...
    r2_base = settings.R2_PUBLIC_URL or "https://pub-7846c786f7154610b57735df47899fa0.r2.dev"
    
    # Map database keys to expected response structure
    result = {
        "logo": config.get("logo_url") or f"{r2_base}/logo.png",
        "video": {
            "intro": config.get("intro_video_url") or f"{r2_base}/Create_a_video_202602141450_ub9p5.mp4"
        },
...
async def list_users(
    ...
    if search:
        where_conditions.append("(u.email ILIKE :search OR u.username ILIKE :search OR up.full_name ILIKE :search)")
...
@app.get("/api/v1/admin/users/{user_id}", response_model=UserListItem, tags=["Admin Users"])
async def get_user(
    user_id: int,
    db: Session = Depends(get_db),
    current_user: dict = Depends(require_admin)
...
async def update_user_status(
    ...
    db.execute(
        ...
        {"is_active": data.is_active, "now": datetime.now(timezone.utc), "user_id": user_id}
    )
    db.commit()
    ...
    return {"message": f"User status updated to {'active' if data.is_active else 'inactive'}"}
...
async def bulk_update_user_status(
    ...
    return {"message": f"Updated {len(data.user_ids)} users to {'active' if data.is_active else 'inactive'}"}
...
async def update_discount(
    ...
    sets, params = ["updated_at = :now"], {"id": discount_id, "now": datetime.now(timezone.utc)}
    if is_active is not None:
...
async def admin_list_products(
    ...
    for r in rows:
        ...
        products.append({
            ...
            "price": float(r[3]) if r[3] else 0,
            "mrp": float(r[4]) if r[4] else None,
            "short_description": r[5], "description": r[6],
            "is_active": r[7], "is_featured": r[8], "is_new_arrival": r[9],
            "category_id": r[10], "collection_id": r[10],
            "collection_name": r[11],
            "meta_title": r[12], "meta_description": r[13],
...
async def admin_create_product(
    ...
    existing = db.execute(text("SELECT id FROM products WHERE slug = :slug"), {"slug": slug}).fetchone()
    if existing:
    ...
    """), {
        ...
        "desc": data.description, "short_desc": data.short_description,
        "price": data.base_price, "mrp": data.mrp,
        "cat_id": category_id, "brand": getattr(data, 'brand', None),
        "active": data.is_active, "featured": data.is_featured,
        "new_arrival": data.is_new_arrival,
        "meta_title": data.meta_title, "meta_desc": data.meta_description,
        "now": datetime.now(timezone.utc)
    })
    ...
    redis_client.invalidate_pattern("products:*")
    redis_client.invalidate_pattern("public:landing:*")
    return {"id": product_id, "name": data.name, "slug": slug, "message": "Product created"}
...
async def admin_update_product(
    ...
    sets, params = ["updated_at = :now"], {"id": product_id, "now": datetime.now(timezone.utc)}
    update_map = {
    ...
    if data.base_price is not None:
        sets.append("base_price = :base_price"); params["base_price"] = data.base_price
    if data.mrp is not None:
        sets.append("mrp = :mrp"); params["mrp"] = data.mrp
    cat_id = getattr(data, 'collection_id', None) or getattr(data, 'category_id', None)
    if cat_id is not None:
        sets.append("category_id = :cat_id"); params["cat_id"] = cat_id
    result = db.execute(
...
@app.delete("/api/v1/admin/products/{product_id}", status_code=204, tags=["Admin Products"])
async def admin_delete_product(
    ...
    db.execute(text("DELETE FROM product_images WHERE product_id = :pid"), {"pid": product_id})
    result = db.execute(
...
async def admin_bulk_price_update(
    ...
    updated = 0
    for p in products:
        sets, params = ["updated_at = :now"], {"id": p[0], "now": datetime.now(timezone.utc)}
        price = Decimal(str(p[1])) if p[1] else Decimal('0')
        mrp = Decimal(str(p[2])) if p[2] else None
        if data.price is not None:
            sets.append("base_price = :price"); params["price"] = data.price
        if data.mrp is not None:
            sets.append("mrp = :mrp"); params["mrp"] = data.mrp
        if data.price_adjustment is not None:
            new_price = max(Decimal('0.01'), price + Decimal(str(data.price_adjustment)))
            sets.append("base_price = :price"); params["price"] = float(new_price)
...
async def admin_bulk_status_update(
    ...
    sets, params = ["updated_at = :now"], {"ids": data.product_ids, "now": datetime.now(timezone.utc)}
    if data.is_active is not None:
        sets.append("is_active = :active"); params["active"] = data.is_active
    if data.is_featured is not None:
        sets.append("is_featured = :featured"); params["featured"] = data.is_featured
    if data.is_new_arrival is not None:
        sets.append("is_new_arrival = :new_arrival"); params["new_arrival"] = data.is_new_arrival
    db.execute(text(f"UPDATE products SET {', '.join(sets)} WHERE id = ANY(:ids)"), params)
    db.commit()
    redis_client.invalidate_pattern("products:*")
    redis_client.invalidate_pattern("public:landing:*")
...
async def admin_bulk_assign_collection(
    ...
    coll = db.execute(text("SELECT id FROM collections WHERE id = :id"), {"id": data.collection_id}).fetchone()
    if not coll:
        raise HTTPException(status_code=404, detail="Collection not found")
    db.execute(
        text("UPDATE products SET category_id = :cid, updated_at = :now WHERE id = ANY(:ids)"),
        {"cid": data.collection_id, "ids": data.product_ids, "now": datetime.now(timezone.utc)}
    )
    db.commit()
    redis_client.invalidate_pattern("products:*")
...
async def admin_bulk_inventory_update(
    ...
    for item in updates:
        ...
        if not sku or qty is None:
            ...
            continue
        result = db.execute(
            text("UPDATE inventory SET quantity = :qty, updated_at = :now WHERE sku = :sku"),
...
async def admin_bulk_delete_products(
    ...
    for pid in product_ids:
        images = db.execute(
            text("SELECT image_url FROM product_images WHERE product_id = :pid"), {"pid": pid}
        ).fetchall()
        for img in images:
            if img[0]:
                await r2_service.delete_image(img[0])
        db.execute(text("DELETE FROM product_images WHERE product_id = :pid"), {"pid": pid})
        result = db.execute(text("DELETE FROM products WHERE id = :id RETURNING id"), {"id": pid})
        if result.fetchone():
            deleted += 1
    db.commit()
    redis_client.invalidate_pattern("products:*")
...
@app.post("/api/v1/admin/products/{product_id}/images", status_code=201, tags=["Admin Products"])
async def admin_upload_product_image(
    ...
    product = db.execute(text("SELECT id, name FROM products WHERE id = :id"), {"id": product_id}).fetchone()
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
    ...
    relative_path = full_url.replace(r2_base + '/', '') if r2_base and full_url.startswith(r2_base) else full_url
    if is_primary:
        db.execute(
    ...
    img_count = db.execute(
        text("SELECT COUNT(*) FROM product_images WHERE product_id = :pid"), {"pid": product_id}
    ).scalar() or 0
    result = db.execute(text("""
    ...
    """), {
        "pid": product_id, "url": relative_path,
        "alt": alt_text or f"{product[1]} - Image {img_count + 1}", "primary": is_primary,
        "order": img_count, "now": datetime.now(timezone.utc)
    })
    ...
    return {
        ...
        "is_primary": is_primary, "display_order": img_count
    }


@app.delete("/api/v1/admin/products/{product_id}/images/{image_id}", status_code=204, tags=["Admin Products"])
async def admin_delete_product_image(
    ...
    img = db.execute(
        text("SELECT image_url FROM product_images WHERE id = :id AND product_id = :pid"),
...
@app.patch("/api/v1/admin/products/{product_id}/images/reorder", tags=["Admin Products"])
async def admin_reorder_product_images(
    ...
    for idx, img_id in enumerate(image_ids):
        db.execute(
            text("UPDATE product_images SET display_order = :order WHERE id = :id AND product_id = :pid"),
...
@app.patch("/api/v1/admin/products/{product_id}/images/{image_id}/primary", tags=["Admin Products"])
...
@app.post("/api/v1/admin/products/{product_id}/variants", status_code=201, tags=["Admin Products"])
async def admin_create_product_variant(
    ...
    if existing:
        raise HTTPException(status_code=400, detail=f"SKU '{data.sku}' already exists for this product")
    result = db.execute(text("""
    ...
    """), {
        ...
        "qty": data.quantity, "now": datetime.now(timezone.utc)
    })
    inv_id = result.scalar()
    db.commit()
    redis_client.invalidate_pattern("products:*")
...
@app.patch("/api/v1/admin/products/{product_id}/variants/{variant_id}", tags=["Admin Products"])
...
@app.delete("/api/v1/admin/products/{product_id}/variants/{variant_id}", status_code=204, tags=["Admin Products"])
...
@app.post("/api/v1/admin/products/{product_id}/variants/{variant_id}/adjust-stock", tags=["Admin Products"])
...
async def admin_list_collections(
    ...
    collections = [{
        ...
        "display_order": r[5], "is_active": r[6], "is_featured": r[7],
        "created_at": r[8], "updated_at": r[9], "product_count": r[10]
    } for r in rows]
    return collections
...
async def admin_create_collection(
    ...
    if not slug:
        slug = re.sub(r'[^a-z0-9]+', '-', data.name.lower()).strip('-')
    existing = db.execute(text("SELECT id FROM collections WHERE slug = :slug"), {"slug": slug}).fetchone()
    if existing:
        slug = f"{slug}-{int(datetime.now(timezone.utc).timestamp())}"
    result = db.execute(text("""
        ...
        RETURNING id
    """), {
        "name": data.name, "slug": slug, "desc": data.description,
        "img": data.image_url, "order": data.display_order,
        "active": data.is_active, "featured": data.is_featured,
        "now": datetime.now(timezone.utc)
    })
    ...
    redis_client.invalidate_pattern("public:landing:*")
    return {"id": coll_id, "name": data.name, "slug": slug, "message": "Collection created"}
...
async def admin_update_collection(
    ...
    sets, params = ["updated_at = :now"], {"id": collection_id, "now": datetime.now(timezone.utc)}
    if data.name is not None: sets.append("name = :name"); params["name"] = data.name
    if data.slug is not None: sets.append("slug = :slug"); params["slug"] = data.slug
    if data.description is not None: sets.append("description = :desc"); params["desc"] = data.description
    if data.image_url is not None: sets.append("image_url = :img"); params["img"] = data.image_url
    if data.display_order is not None: sets.append("display_order = :order"); params["order"] = data.display_order
    if data.is_active is not None: sets.append("is_active = :active"); params["active"] = data.is_active
    if data.is_featured is not None: sets.append("is_featured = :featured"); params["featured"] = data.is_featured
    result = db.execute(
        text(f"UPDATE collections SET {', '.join(sets)} WHERE id = :id RETURNING id"), params
    )
    if not result.fetchone():
        raise HTTPException(status_code=404, detail="Collection not found")
    db.commit()
...
@app.delete("/api/v1/admin/collections/{collection_id}", status_code=204, tags=["Admin Collections"])
@app.delete("/api/v1/admin/categories/{collection_id}", status_code=204, tags=["Admin Collections"])
async def admin_delete_collection(
    ...
    product_count = db.execute(
        text("SELECT COUNT(*) FROM products WHERE category_id = :id"), {"id": collection_id}
    ).scalar() or 0
    if product_count > 0:
        raise HTTPException(
            status_code=400,
    ...
    result = db.execute(
        text("DELETE FROM collections WHERE id = :id RETURNING id"), {"id": collection_id}
    )
    if not result.fetchone():
        raise HTTPException(status_code=404, detail="Collection not found")
    db.commit()
...
async def admin_bulk_collection_status(
    ...
    db.execute(
        text("UPDATE collections SET is_active = :active, updated_at = :now WHERE id = ANY(:ids)"),
...
async def admin_bulk_collection_reorder(
    ...
    for item in data.items:
        db.execute(
            text("UPDATE collections SET display_order = :order, updated_at = :now WHERE id = :id"),
            {"order": item["display_order"], "id": item["id"], "now": datetime.now(timezone.utc)}
        )
    db.commit()
    redis_client.invalidate_pattern("collections:*")
    redis_client.invalidate_pattern("public:landing:*")
...
async def admin_upload_collection_image(
    ...
    coll = db.execute(
        text("SELECT id, image_url FROM collections WHERE id = :id"), {"id": category_id}
    ).fetchone()
    if not coll:
        raise HTTPException(status_code=404, detail="Collection not found")
    if coll[1]:
    ...
    relative_path = full_url.replace(r2_base + '/', '') if r2_base and full_url.startswith(r2_base) else full_url
    db.execute(
        text("UPDATE collections SET image_url = :img, updated_at = :now WHERE id = :id"),
        {"img": relative_path, "id": category_id, "now": datetime.now(timezone.utc)}
    )
    db.commit()
    redis_client.invalidate_pattern("collections:*")
    redis_client.invalidate_pattern("public:landing:*")
    return {"message": "Collection image updated", "image_url": _get_r2_public_url(relative_path)}


@app.delete("/api/v1/admin/categories/{category_id}/image", status_code=204, tags=["Admin Collections"])
async def admin_delete_collection_image(
    ...
    if coll[1]:
        await r2_service.delete_image(coll[1])
        db.execute(
            text("UPDATE collections SET image_url = NULL, updated_at = :now WHERE id = :id"),
...
async def admin_create_inventory(
    ...
    """), {
        ...
        "qty": data["quantity"], "threshold": data.get("low_stock_threshold", 5),
        "cost": data.get("cost_price"), "now": datetime.now(timezone.utc)
    })
    inv_id = result.scalar()
    db.commit()
    redis_client.invalidate_pattern("products:*")
...
async def admin_update_inventory(
    ...
    for field in ALLOWED:
        if field in data:
            sets.append(f"{field} = :{field}"); params[field] = data[field]
    if len(sets) == 1:
        return {"message": "Nothing to update"}
    db.execute(text(f"UPDATE inventory SET {', '.join(sets)} WHERE id = :id"), params)
    db.commit()
...
async def admin_approve_return(
    ...
    if refund_amount is not None:
        sets.append("refund_amount = :refund"); params["refund"] = refund_amount
...
async def admin_receive_return(
    ...
    if tracking:
        sets.append("return_tracking_number = :tn"); params["tn"] = tracking
    db.execute(text(f"UPDATE return_requests SET {', '.join(sets)} WHERE id = :id"), params)
    db.commit()
...
async def admin_list_reviews(
    ...
    """), params).fetchall()
    reviews = [{
        "id": r[0], "product_id": r[1], "user_id": r[2],
        "rating": r[3], "comment": r[4], "is_approved": r[5], "created_at": r[6],
...
async def admin_approve_review(
    ...
    result = db.execute(
        text("UPDATE reviews SET is_approved = true, updated_at = :now WHERE id = :id RETURNING id"),
...
async def admin_reject_review(
    ...
    result = db.execute(
        text("UPDATE reviews SET is_approved = false, updated_at = :now WHERE id = :id RETURNING id"),
        {"id": review_id, "now": datetime.now(timezone.utc)}
    )
    if not result.fetchone():
        raise HTTPException(status_code=404, detail="Review not found")
...
@app.delete("/api/v1/admin/reviews/{review_id}", status_code=204, tags=["Admin Reviews"])
