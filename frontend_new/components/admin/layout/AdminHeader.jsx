...
export default function AdminHeader({ onMenuClick, user, mobileMenuOpen = false }) {
  ...
  return (
    <header
      className="h-16 bg-[#111111]/80 backdrop-blur-xl border-b border-[#A8B4C8]/15 sticky top-0 z-30"
      role="banner"
    >
      <div className="h-full px-2 sm:px-4 flex items-center justify-between gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="lg:hidden p-2 rounded-lg hover:bg-[#A8B4C8]/10 transition-colors min-w-[44px] min-h-[44px] touch-target"
            aria-label="Open navigation menu"
            aria-expanded={mobileMenuOpen}
            aria-controls="mobile-sidebar"
          >
            <Menu className="w-5 h-5 text-[#F5F0E8]/70" />
          </button>

          {/* Search Bar */}
          <div className="relative w-full max-w-[200px] sm:max-w-xs md:max-w-sm lg:max-w-md xl:max-w-lg">
            <Search
              className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#F5F0E8]/40"
              aria-hidden="true"
            />
            <input
              type="search"
              ...
              className={cn(
                ...
                'focus:outline-none focus:border-[#A8B4C8]/40 focus:ring-2 focus:ring-[#A8B4C8]/20',
        ...
        <div className="flex items-center gap-3">
          {/* Notifications */}
          <button
            className="relative p-2 rounded-xl hover:bg-[#A8B4C8]/10 transition-colors min-w-[44px] min-h-[44px] touch-target"
            aria-label="Notifications"
            aria-describedby="notification-count"
          >
            <Bell className="w-5 h-5 text-[#F5F0E8]/70" />
            <span
              id="notification-count"
              className="absolute top-1 right-1 w-2 h-2 bg-[#A8B4C8] rounded-full"
          ...
          <div className="relative" ref={userMenuRef}>
            <button
              ...
              className={cn(
                'flex items-center gap-2 p-2 rounded-xl hover:bg-[#A8B4C8]/10 transition-colors',
                'min-h-[44px] touch-target'
              )}
            ...
            >
              <div
                className="w-8 h-8 rounded-full bg-[#1E3A5F]/30 border border-[#A8B4C8]/30 flex items-center justify-center"
                aria-hidden="true"
              >
              ...
              <div className="hidden md:block text-left">
                ...
                <p className="text-xs text-[#F5F0E8]/50 capitalize">
                  {user?.role || 'admin'}
                </p>
              </div>
              <ChevronDown
                className={cn(
                  'w-4 h-4 text-[#F5F0E8]/50 hidden md:block transition-transform',
            ...
            {showUserMenu && (
              ...
              >
                <Link
                  ...
                  className={cn(
                    'flex items-center gap-2 px-4 py-2 text-sm text-[#F5F0E8]/70',
                    'hover:bg-[#A8B4C8]/10 hover:text-[#F5F0E8]',
                    'transition-colors',
                    'min-h-[44px] touch-target'
                  )}
                  role="menuitem"
                ...
                <hr className="my-2 border-[#A8B4C8]/15" role="separator" />
                <button
