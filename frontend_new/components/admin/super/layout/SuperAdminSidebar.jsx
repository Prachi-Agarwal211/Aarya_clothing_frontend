  ...
  return (
    ...
    >
      ...
      <div className="h-16 flex items-center justify-between px-4 border-b border-[#E07B8B]/15">
        {!collapsed && (
          <Link href="/admin/super" className="flex items-center gap-2">
            ...
            <span className="text-sm text-[#F5F5F5]/60">Super Admin</span>
          </Link>
        )}
        <button
          onClick={onToggle}
          className="p-2 rounded-lg hover:bg-[#E07B8B]/10 transition-colors"
        >
          {collapsed ? (
      ...
      <nav className="flex-1 py-4 overflow-y-auto">
          ...
          {navigation.map((item) => {
            ...
            return (
              <li key={item.name}>
                <Link
                  href={item.href}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 ${active ? 'bg-[#9333EA]/30 text-[#FFD700] border border-[#E07B8B]/30' : 'text-[#F5F5F5]/70 hover:bg-[#E07B8B]/10 hover:text-[#F5F5F5]'}`}
                  title={collapsed ? item.name : undefined}
                >
                  <Icon className={`w-5 h-5 flex-shrink-0 ${active ? 'text-[#FFD700]' : ''}`} />
        ...
        {!collapsed && (
          <p className="px-4 pt-4 pb-1 text-xs text-[#F5F5F5]/30 uppercase tracking-widest">Admin</p>
        )}
        <ul className="space-y-1 px-2">
