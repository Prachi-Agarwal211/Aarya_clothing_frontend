...
async def create_staff_account(
    ...
    # Create user — force role to 'staff' for security (only super_admin creates admin accounts)
    safe_role = account_data.role if account_data.role in ('staff',) else 'staff'
    if user.get('role') == 'super_admin':
        safe_role = account_data.role if account_data.role in ('staff', 'admin') else 'staff'

    result = db.execute(
        text("""
            INSERT INTO users (email, username, hashed_password, role, full_name, phone, department, custom_role_id, is_active)
            VALUES (:email, :username, :hashed_password, :role, :full_name, :phone, :department, :custom_role_id, :is_active)
            RETURNING id, email, username, role, full_name, phone, department, custom_role_id, is_active, created_at
        """),
        {
            "email": account_data.email,
            "username": account_data.username,
    ...
    return {
        ...
        "custom_role_id": result[7],
        "custom_role_name": None,
        "permissions": permissions,
        "phone": result[5] or "",
        "department": result[6] or "",
        "is_active": result[8],
