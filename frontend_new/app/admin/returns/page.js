  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { statusKey: RETURN_STATUS.REQUESTED, label: 'Requested', count: stats.requested, color: 'text-amber-400', border: 'border-amber-500/20', bg: 'bg-amber-500/10', activeBg: 'bg-amber-500/20', activeBorder: 'border-amber-500/40' },
          { statusKey: RETURN_STATUS.APPROVED, label: 'Approved', count: stats.approved, color: 'text-blue-400', border: 'border-blue-500/20', bg: 'bg-blue-500/10', activeBg: 'bg-blue-500/20', activeBorder: 'border-blue-500/40' },
          { statusKey: RETURN_STATUS.RECEIVED, label: 'Received', count: stats.received, color: 'text-purple-400', border: 'border-purple-500/20', bg: 'bg-purple-500/10', activeBg: 'bg-purple-500/20', activeBorder: 'border-purple-500/40' },
          { statusKey: RETURN_STATUS.REFUNDED, label: 'Refunded', count: stats.refunded, color: 'text-green-400', border: 'border-green-500/20', bg: 'bg-green-500/10', activeBg: 'bg-green-500/20', activeBorder: 'border-green-500/40' },
        ].map(({ statusKey, label, count, color, border, bg, activeBg, activeBorder }) => {
          ...
          return (
            <button
              ...
              className={`p-4 rounded-2xl border transition-all text-left ${isActive
                  ...
                  : `bg-[#0B0608]/40 ${border} hover:${activeBg}`
                }`}
            >
              <p className={`text-2xl font-bold ${color}`}>{count}</p>
              <p className={`text-xs mt-0.5 font-medium ${isActive ? color : 'text-[#EAE0D5]/60'}`}>{label}</p>
            </button>
          );
        })}
      </div>
