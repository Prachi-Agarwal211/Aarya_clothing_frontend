...
test.describe('Page Navigation', () => {
  test('navigate to new arrivals section on homepage', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');

    // Navigate to new arrivals section (now on homepage, not separate page)
    await page.goto('/#new-arrivals');
    await page.waitForLoadState('networkidle');

    // Verify homepage loaded with new arrivals section visible
    const newArrivalsSection = page.locator('#new-arrivals');
    await expect(newArrivalsSection).toBeVisible();

    // Check product images load in new arrivals section
    const productImages = newArrivalsSection.locator('img');
    const count = await productImages.count();
