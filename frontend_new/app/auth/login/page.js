...
function getLoginErrorMessage(err) {
  if (err.status === 401) return 'Invalid credentials. Please check your username/email and password.';
...
function LoginForm() {
  ...
  return (
    <div className="w-full max-w-[480px] lg:max-w-[520px] xl:max-w-[560px] flex flex-col items-center">
      ...
      {/* Redirect notice */}
      {redirectUrl && (
        <div className="w-full mb-4 p-3 bg-[#7A2F57]/20 border border-[#B76E79]/30 rounded-xl text-center">
      ...
      <form className="w-full space-y-5 sm:space-y-6 md:space-y-7 animate-fade-in-up-delay" onSubmit={handleSubmit}>
        ...
        <div className="flex items-center justify-between">
          ...
          <Link
            ...
            className="text-xs sm:text-sm md:text-base uppercase tracking-widest text-white/80 hover:text-[#F2C29A] transition-colors"
          >
            Forgot Password?
          </Link>
        </div>
