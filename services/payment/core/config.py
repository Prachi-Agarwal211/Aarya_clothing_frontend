...
class Settings(SharedBaseSettings):
    ...
    CASHFREE_ENV: str = "SANDBOX"  # PRODUCTION | SANDBOX
    ...
    # ==================== Easebuzz Configuration (legacy) ====================
    EASEBUZZ_MERCHANT_KEY: Optional[str] = None
    EASEBUZZ_SALT: Optional[str] = None
    EASEBUZZ_ENV: str = "test"  # test / production
    EASEBUZZ_WEBHOOK_SECRET: Optional[str] = None
    ...
    @property
    def easebuzz_enabled(self) -> bool:
        """Check if Easebuzz is configured."""
        return bool(
            self.EASEBUZZ_MERCHANT_KEY and
            self.EASEBUZZ_SALT
        )
