...
export default function ProductForm({
  ...
  const inputCls = (field) =>
    `w-full px-4 py-2.5 bg-[#111111]/60 border rounded-xl text-[#F5F0E8] placeholder-[#F5F0E8]/40 focus:outline-none transition-colors ${
      errors[field] ? 'border-red-500/50' : 'border-[#A8B4C8]/20 focus:border-[#A8B4C8]/40'
    }`;

  const cardCls = 'bg-[#111111]/40 backdrop-blur-md border border-[#A8B4C8]/15 rounded-2xl p-6';
  const heading = 'text-lg font-semibold text-[#D4AF37]';
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[#F5F0E8]/60 mt-1 text-sm">
            {isEdit
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm text-[#F5F0E8]/70 mb-1">
                    URL Slug
                    <span className="text-[#F5F0E8]/40 text-xs ml-2">
                ...
                <div>
                  <label className="block text-sm text-[#F5F0E8]/70 mb-2">Description</label>
                  <textarea
                    name="description"
                    ...
                    rows={6}
                    className="w-full px-4 py-2.5 bg-[#111111]/60 border border-[#A8B4C8]/20 rounded-xl text-[#F5F0E8] placeholder-[#F5F0E8]/40 focus:outline-none focus:border-[#A8B4C8]/40 transition-colors resize-y"
                  />
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#F5F0E8]/50">₹</span>
                    <input
                      type="number"
                      name="price"
                      value={form.price}
                ...
                <div>
                  <label className="block text-sm text-[#F5F0E8]/70 mb-1">
                    MRP
                    <span className="text-[#F5F0E8]/40 text-xs ml-2">≥ selling price</span>
                  </label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#F5F0E8]/50">₹</span>
                    <input
                ...
                <div className="p-3 bg-[#1E3A5F]/10 border border-[#A8B4C8]/20 rounded-xl text-xs text-[#F5F0E8]/70">
                  Total stock across variants:{' '}
                  <span className="text-[#D4AF37] font-semibold">{totalStock}</span>
                </div>
              </div>
            </div>
              ...
              {images.length > 0 && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                  {images.map((img, idx) => (
                    <div
                      key={img.id || `new-${idx}`}
                      className="relative aspect-square rounded-xl overflow-hidden bg-[#1E3A5F]/10 border border-[#A8B4C8]/30 group"
                    >
                      <Image
                        src={img.preview}
                        alt={`Image ${idx + 1}`}
                      ...
                      />
                      {idx === primaryIndex && (
                        <span className="absolute top-2 left-2 bg-[#1E3A5F] text-[#D4AF37] text-[10px] px-2 py-0.5 rounded shadow font-semibold">
                          Primary
                        </span>
                      )}
                      <button
                        ...
                        className={`absolute bottom-2 left-2 p-1 rounded-lg transition-all ${
                          idx === primaryIndex
                            ...
                            : 'text-white/60 opacity-0 group-hover:opacity-100 hover:text-[#D4AF37]'
                        }`}
                      >
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  type="button"
                  onClick={addVariant}
                  className="flex items-center gap-1 text-sm bg-[#A8B4C8]/20 px-3 py-1.5 rounded-lg text-[#D4AF37] hover:bg-[#A8B4C8]/40 transition-colors"
                >
              ...
              {variants.length > 0 ? (
                ...
                <div className="p-8 border border-dashed border-[#A8B4C8]/20 rounded-xl text-center">
                  <p className="text-sm text-[#F5F0E8]/50">
                    No variants yet. Click <span className="text-[#D4AF37]">Add Variant</span> to
          ...
          <div className="space-y-6">
                ...
                {[
                  ...
                  <label key={name} className="flex items-start gap-3 cursor-pointer">
                    <input
                      ...
                      checked={!!form[name]}
                      onChange={handleField}
                      className="w-5 h-5 mt-0.5 rounded border-[#A8B4C8]/30 bg-[#111111]/60 text-[#A8B4C8] focus:ring-[#A8B4C8]/30"
                    />
                    <div>
                      <span className="text-[#F5F0E8] text-sm">{label}</span>
                      <p className="text-[#F5F0E8]/40 text-xs">{hint}</p>
                    </div>
                  </label>
                ))}
              </div>
            ...
            <div className="flex flex-col gap-3">
              <button
                type="submit"
                disabled={submitting}
                className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-[#1E3A5F]/50 border border-[#A8B4C8]/50 rounded-xl text-[#D4AF37] hover:bg-[#1E3A5F]/70 transition-all disabled:opacity-50"
              >
              ...
              <Link
                href="/admin/products"
                className="w-full px-4 py-2.5 border border-[#A8B4C8]/20 rounded-xl text-[#F5F0E8]/70 hover:bg-[#A8B4C8]/10 transition-colors text-center"
              >
                Cancel
              </Link>
            </div>
