...
class OrderStatusUpdate(BaseModel):
    status: str
    pod_number: Optional[str] = None       # POD = Proof of Delivery / tracking number. Required when status=shipped.
...
class ProductCreate(BaseModel):
    ...
    base_price: float = Field(..., gt=0, description="Selling price must be greater than 0")
    mrp: Optional[float] = Field(None, ge=0, description="MRP cannot be negative")
    ...
    def mrp_gte_base_price(self):
        if self.mrp is not None and self.mrp < self.base_price:
...
class ProductUpdate(BaseModel):
    ...
    description: Optional[str] = None
    short_description: Optional[str] = None
    base_price: Optional[float] = Field(None, gt=0, description="Selling price must be greater than 0")
    mrp: Optional[float] = Field(None, ge=0, description="MRP cannot be negative")
    category_id: Optional[int] = None
    collection_id: Optional[int] = None
    brand: Optional[str] = None
    ...
    @model_validator(mode='after')
    def mrp_gte_base_price(self):
        if self.mrp is not None and self.base_price is not None and self.mrp < self.base_price:
            raise ValueError('MRP must be greater than or equal to selling price')
        return self
