...
class AuthMiddleware:
    ...
    def decode_token(self, token: str, expected_type: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            return self.token_validator.validate_token(token, expected_type=expected_type)
    ...
    def validate_token(self, credentials: Optional[HTTPAuthorizationCredentials], expected_type: Optional[str] = None) -> Dict[str, Any]:
...
def initialize_auth_middleware(secret_key: str, algorithm: str = "HS256", redis_client=None):
...
def get_current_user_optional(request: Request, credentials: Optional[HTTPAuthorizationCredentials] = Depends(security)) -> Optional[Dict[str, Any]]:
