    ...
    <div className="min-h-screen bg-[#0B0608] flex flex-col items-center justify-center px-4 py-12"
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="space-y-3">
          <button onClick={() => router.push('/auth/login?redirect=/ai')}
          ...
          <button onClick={() => router.push('/auth/register?redirect=/ai')}
...
export default function AiShopPage() {
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    setLoading(true);
    try {
      const res = await aiApi.customerChat(msg, sessionId, language);
      if (res.session_id) setSessionId(res.session_id);
      setMessages(prev => [...prev, {
        id: Date.now() + 1, role: 'assistant',
        text: res.reply || "I'm here to help! What would you like to see?",
        toolResults: res.tool_results || null,
        timestamp: new Date(),
      }]);
    } catch {
      setMessages(prev => [...prev, {
  ...
  }, [input, sessionId, loading, language]);
      ...
      <div className="flex-shrink-0 bg-[#0A0608]/98 backdrop-blur-xl border-t border-[#B76E79]/15">
        <div className="max-w-2xl mx-auto px-4 pt-3 pb-safe pb-4">
          ...
          <p className="text-center text-[10px] text-[#EAE0D5]/15 mt-2">
            Aarya AI · Powered by Gemini · Fashion-focused
          </p>
        </div>
      </div>
    </div>
