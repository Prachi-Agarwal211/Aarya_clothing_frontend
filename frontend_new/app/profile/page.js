  ...
  if (authLoading) {
    return (
      <div className="animate-pulse space-y-6">
        <div className="h-48 bg-[#E07B8B]/10 rounded-2xl" />
        <div className="h-32 bg-[#E07B8B]/10 rounded-2xl" />
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="p-6 bg-[#0A0A0A]/40 backdrop-blur-md border border-[#E07B8B]/15 rounded-2xl">
        <div className="flex items-start justify-between mb-6">
          <h2 className="text-xl font-semibold text-[#FFD700]">Profile Information</h2>
          {!editing && (
            <button
              onClick={() => setEditing(true)}
              className="flex items-center gap-2 px-4 py-2 text-sm text-[#E07B8B] hover:text-[#FFD700] border border-[#E07B8B]/20 rounded-lg hover:border-[#E07B8B]/40 transition-colors"
            >
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-shrink-0">
            <div className="relative">
              <div className="w-24 h-24 rounded-full bg-gradient-to-r from-[#9333EA] to-[#E07B8B] flex items-center justify-center">
                <User className="w-12 h-12 text-white" />
              </div>
              {editing && (
                <button className="absolute bottom-0 right-0 w-8 h-8 bg-[#E07B8B] rounded-full flex items-center justify-center text-white hover:bg-[#9333EA] transition-colors">
            ...
            {editing ? (
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 bg-[#0A0A0A]/60 border border-[#E07B8B]/20 rounded-lg text-[#F5F5F5] focus:outline-none focus:border-[#E07B8B]/40"
                  />
                </div>
                <div>
                  <label className="block text-sm text-[#F5F5F5]/70 mb-1">Email</label>
                  <input
                    type="email"
                    value={formData.email}
                    disabled
                    className="w-full px-3 py-2.5 bg-[#0A0A0A]/60 border border-[#E07B8B]/20 rounded-lg text-[#F5F5F5]/60 focus:outline-none"
                  />
                </div>
                <div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-lg text-[#FFD700]">{user?.profile?.full_name || user?.full_name || user?.username}</span>
                </div>
                <div className="flex items-center gap-3">
                  <Mail className="w-5 h-5 text-[#E07B8B]" />
                  <span className="text-[#F5F5F5]">{user?.email}</span>
                </div>
                <div className="flex items-center gap-3">
                  <Phone className="w-5 h-5 text-[#E07B8B]" />
                  <span className="text-[#F5F5F5]">{user?.profile?.phone || user?.phone}</span>
                </div>
                <div className="flex items-center gap-3">
                  <Calendar className="w-5 h-5 text-[#E07B8B]" />
                  <span className="text-[#F5F5F5]/70">Member since {formatDate(user?.created_at || '2024-01-01')}</span>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="p-6 bg-[#0A0A0A]/40 backdrop-blur-md border border-[#E07B8B]/15 rounded-2xl">
        <div className="flex items-center gap-2 mb-4">
          <ShieldCheck className="w-5 h-5 text-[#E07B8B]" />
          <h3 className="text-lg font-semibold text-[#FFD700]">Account verification</h3>
        </div>
        ...
        {verifyMsg ? (
          <div className="mb-3 text-sm text-[#E07B8B]/90">{verifyMsg}</div>
        ) : null}

        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 py-2 border-b border-[#E07B8B]/10">
            <div>
            ...
            <div className="flex flex-wrap gap-2">
              {emailVerified ? (
                ...
                <button
                  ...
                  disabled={!!verifyBusy}
                  className="text-sm px-3 py-1.5 rounded-lg bg-[#E07B8B]/20 text-[#FFD700] hover:bg-[#E07B8B]/30 disabled:opacity-50"
                >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 py-2 border-b border-[#E07B8B]/10">
            <div>
              <p className="text-[#F5F5F5]">Phone</p>
              <p className="text-xs text-[#F5F5F5]/50">
                {phoneVerified
                  ? 'Verified'
          ...
          {otpChannel ? (
            <div className="flex flex-col sm:flex-row gap-2 sm:items-end pt-2">
              <div className="flex-1">
                <label className="block text-xs text-[#F5F5F5]/60 mb-1">6-digit code</label>
                <input
                  type="text"
                  inputMode="numeric"
                  ...
                  className="w-full sm:max-w-xs px-3 py-2 bg-[#0A0A0A]/60 border border-[#E07B8B]/20 rounded-lg text-[#F5F5F5] tracking-widest"
                  placeholder="••••••"
                />
              </div>
              <button
                ...
                className="px-4 py-2 rounded-lg bg-gradient-to-r from-[#9333EA] to-[#E07B8B] text-white text-sm disabled:opacity-50"
              >
              ...
              <button
                ...
                className="text-sm text-[#F5F5F5]/60 hover:text-[#F5F5F5]"
              >
                Cancel
              </button>
            </div>
      ...
      {/* Account Actions */}
      <div className="p-6 bg-[#0A0A0A]/40 backdrop-blur-md border border-[#E07B8B]/15 rounded-2xl">
        <h3 className="text-lg font-semibold text-[#FFD700] mb-4">Account Actions</h3>
        <div className="space-y-4">
          <div className="py-3 border-b border-[#E07B8B]/10">
            <p className="text-[#F5F5F5]">Order updates</p>
          ...
          <div className="py-3 border-b border-[#E07B8B]/10">
            <button
              ...
              disabled={loggingOut}
              className="flex items-center gap-2 px-4 py-2.5 w-full sm:w-auto border border-[#E07B8B]/30 text-[#E07B8B] rounded-xl hover:border-[#E07B8B]/60 hover:text-[#FFD700] transition-colors disabled:opacity-50"
            >
          ...
          <div className="pt-2">
            <button className="text-sm text-[#E07B8B] hover:text-[#FFD700] transition-colors">
              Delete Account
            </button>
          </div>
        </div>
