...
const CollectionCard = ({ collection, category, className }) => {
  // Accept either `collection` (canonical) or `category` (legacy) prop
  const data = collection || category || {};
  const name = data.name;
  const image = data.image_url || data.image || '';
  const link =
    (data.slug && `/collections/${data.slug}`) ||
    data.link ||
    (data.id != null ? `/products?collection_id=${data.id}` : '#');
...
export const CategoryCard = CollectionCard;
