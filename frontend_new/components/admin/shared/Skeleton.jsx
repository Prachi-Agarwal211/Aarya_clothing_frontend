    ...
    >
      <div className="animate-pulse">
        ...
        {[...Array(rows)].map((_, i) => (
          <div
            key={i}
            className="h-14 border-b border-[#A8B4C8]/10 flex items-center px-4 gap-4"
          >
            {[...Array(columns)].map((_, j) => (
              <div
                key={j}
                className={cn(
                  'h-4 bg-[#A8B4C8]/10 rounded',
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {[...Array(count)].map((_, i) => (
        <div
          key={i}
          className="bg-[#111111]/40 backdrop-blur-md border border-[#A8B4C8]/15 rounded-2xl p-4 animate-pulse"
        >
