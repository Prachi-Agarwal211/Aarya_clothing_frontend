...
export default function SuperAdminAIDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Load insights
      const insightsRes = await aiDashboardApi.getInsights('all');
      setInsights(insightsRes);

      // Load pending actions
      const actionsRes = await aiDashboardApi.getPendingActions('pending');
      setPendingActions(actionsRes.actions || []);

      // Load sales metrics
      const salesRes = await aiDashboardApi.executeQuery('get_sales_metrics', { period: 'today', compare_previous: true });
      
      // Load inventory status
      const inventoryRes = await aiDashboardApi.executeQuery('get_inventory_status', { alert_threshold: 10 });

      // Load customer analytics
      const customersRes = await aiDashboardApi.executeQuery('get_customer_analytics', { period: 'week', limit: 5 });

      // Load order fulfillment
      const ordersRes = await aiDashboardApi.executeQuery('get_order_fulfillment', { status: 'all', limit: 10 });

      // Load revenue trends
      const trendsRes = await aiDashboardApi.executeQuery('get_revenue_trends', { granularity: 'daily', days: 7 });

      setDashboardData({
        sales: JSON.parse(salesRes.result),
        inventory: JSON.parse(inventoryRes.result),
        customers: JSON.parse(customersRes.result),
        orders: JSON.parse(ordersRes.result),
        trends: JSON.parse(trendsRes.result),
      });
    } catch (error) {
