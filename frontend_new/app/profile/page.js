  ...
  if (authLoading) {
    return (
      <div className="animate-pulse space-y-6">
        <div className="h-48 bg-[#A8B4C8]/10 rounded-2xl" />
        <div className="h-32 bg-[#A8B4C8]/10 rounded-2xl" />
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="p-6 bg-[#111111]/40 backdrop-blur-md border border-[#A8B4C8]/15 rounded-2xl">
        <div className="flex items-start justify-between mb-6">
          <h2 className="text-xl font-semibold text-[#D4AF37]">Profile Information</h2>
          {!editing && (
            <button
              onClick={() => setEditing(true)}
              className="flex items-center gap-2 px-4 py-2 text-sm text-[#A8B4C8] hover:text-[#D4AF37] border border-[#A8B4C8]/20 rounded-lg hover:border-[#A8B4C8]/40 transition-colors"
            >
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-shrink-0">
            <div className="relative">
              <div className="w-24 h-24 rounded-full bg-gradient-to-r from-[#1E3A5F] to-[#A8B4C8] flex items-center justify-center">
                <User className="w-12 h-12 text-white" />
              </div>
              {editing && (
                <button className="absolute bottom-0 right-0 w-8 h-8 bg-[#A8B4C8] rounded-full flex items-center justify-center text-white hover:bg-[#1E3A5F] transition-colors">
            ...
            {editing ? (
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 bg-[#111111]/60 border border-[#A8B4C8]/20 rounded-lg text-[#F5F0E8] focus:outline-none focus:border-[#A8B4C8]/40"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    disabled
                    className="w-full px-3 py-2.5 bg-[#111111]/60 border border-[#A8B4C8]/20 rounded-lg text-[#F5F0E8]/60 focus:outline-none"
                  />
                </div>
                <div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-lg text-[#D4AF37]">{user?.profile?.full_name || user?.full_name || user?.username}</span>
                </div>
                <div className="flex items-center gap-3">
                  <Mail className="w-5 h-5 text-[#A8B4C8]" />
                  <span className="text-[#F5F0E8]">{user?.email}</span>
                </div>
                <div className="flex items-center gap-3">
                  <Phone className="w-5 h-5 text-[#A8B4C8]" />
                  <span className="text-[#F5F0E8]">{user?.profile?.phone || user?.phone}</span>
                </div>
                <div className="flex items-center gap-3">
                  <Calendar className="w-5 h-5 text-[#A8B4C8]" />
                  <span className="text-[#F5F0E8]/70">Member since {formatDate(user?.created_at || '2024-01-01')}</span>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="p-6 bg-[#111111]/40 backdrop-blur-md border border-[#A8B4C8]/15 rounded-2xl">
        <div className="flex items-center gap-2 mb-4">
          <ShieldCheck className="w-5 h-5 text-[#A8B4C8]" />
          <h3 className="text-lg font-semibold text-[#D4AF37]">Account verification</h3>
        </div>
        ...
        {verifyMsg ? (
          <div className="mb-3 text-sm text-[#A8B4C8]/90">{verifyMsg}</div>
        ) : null}

        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 py-2 border-b border-[#A8B4C8]/10">
            <div>
              <p className="text-[#F5F0E8]">Email</p>
            ...
            <div className="flex flex-wrap gap-2">
              {emailVerified ? (
                ...
                <button
                  ...
                  disabled={!!verifyBusy}
                  className="text-sm px-3 py-1.5 rounded-lg bg-[#A8B4C8]/20 text-[#D4AF37] hover:bg-[#A8B4C8]/30 disabled:opacity-50"
                >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 py-2 border-b border-[#A8B4C8]/10">
            <div>
              <p className="text-[#F5F0E8]">Phone</p>
              <p className="text-xs text-[#F5F0E8]/50">
                {phoneVerified
                  ? 'Verified'
          ...
          {otpChannel ? (
            <div className="flex flex-col sm:flex-row gap-2 sm:items-end pt-2">
              <div className="flex-1">
                <label className="block text-xs text-[#F5F0E8]/60 mb-1">6-digit code</label>
                <input
                  type="text"
                  inputMode="numeric"
                  ...
                  className="w-full sm:max-w-xs px-3 py-2 bg-[#111111]/60 border border-[#A8B4C8]/20 rounded-lg text-[#F5F0E8] tracking-widest"
                  placeholder="••••••"
                />
              </div>
              <button
                ...
                className="px-4 py-2 rounded-lg bg-gradient-to-r from-[#1E3A5F] to-[#A8B4C8] text-white text-sm disabled:opacity-50"
              >
              ...
              <button
                ...
                className="text-sm text-[#F5F0E8]/60 hover:text-[#F5F0E8]"
              >
                Cancel
              </button>
            </div>
      ...
      {/* Account Actions */}
      <div className="p-6 bg-[#111111]/40 backdrop-blur-md border border-[#A8B4C8]/15 rounded-2xl">
        <h3 className="text-lg font-semibold text-[#D4AF37] mb-4">Account Actions</h3>
        <div className="space-y-4">
          <div className="py-3 border-b border-[#A8B4C8]/10">
            <p className="text-[#F5F0E8]">Order updates</p>
          ...
          <div className="py-3 border-b border-[#A8B4C8]/10">
            <button
              ...
              disabled={loggingOut}
              className="flex items-center gap-2 px-4 py-2.5 w-full sm:w-auto border border-[#A8B4C8]/30 text-[#A8B4C8] rounded-xl hover:border-[#A8B4C8]/60 hover:text-[#D4AF37] transition-colors disabled:opacity-50"
            >
          ...
          <div className="pt-2">
            <button className="text-sm text-[#A8B4C8] hover:text-[#D4AF37] transition-colors">
              Delete Account
            </button>
          </div>
        </div>
