...
async def create_order_from_payment_webhook(
    ...
    try:
        ...
        return {
            "success": True,
            "order_id": order.id,
            "order": _enrich_order_response(order),
        }

    except ValueError as e:
        logger.error(
            f"INTERNAL_ORDER_CREATE_VALIDATION_ERROR: user={user_id} error={str(e)}"
        )
        ...
        logger.error(
            f"INTERNAL_ORDER_CREATE_ERROR: user={user_id} error={str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail=f"Internal order creation failed: {str(e)}"
        )
