...
def setup_exception_handlers(app: FastAPI):
    ...
    async def core_service_handler(request: Request, exc: CoreServiceException):
        """Handle custom core service exceptions."""
        logger.error(f"Core service error: {exc.message} - Path: {request.url.path}")
...
def log_authentication_event(event_type: str, details: dict, email: str = None):
    ...
    logger.info(f"AUTH EVENT: {auth_data}")
