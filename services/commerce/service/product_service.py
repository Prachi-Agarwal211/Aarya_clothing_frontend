    ...
    def create_product(self, product_data: ProductCreate) -> Product:
        ...
        if product_data.category_id:
            collection = self.db.query(Collection).filter(
                Collection.id == product_data.category_id
            ).first()
            if not collection:
                raise HTTPException(
        ...
        # 🔥 CRITICAL FIX: ALWAYS create at least one inventory record for every product
        # This ensures products are always purchasable and visible in inventory management
        try:
            from models.inventory import Inventory
            variants = getattr(product_data, 'variants', None) or []
            ...
            if variants and len(variants) > 0:
                ...
                for i, v in enumerate(variants):
                    auto_sku = (
                        v.sku or
                        f"PRD-{product.id}-{(v.color or 'STD')[:3].upper()}-{(v.size or 'STD').upper()}-{i}"
                    )
                    inv = Inventory(
            ...
            # Don't fail the whole operation — inventory can be added via admin later
        except SQLAlchemyError as e:
