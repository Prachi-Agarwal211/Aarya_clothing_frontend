      ...
      <div className="relative bg-[#111111]/95 backdrop-blur-xl border border-[#A8B4C8]/20 rounded-2xl p-6 w-full max-w-md">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={handleClose}
            disabled={busy}
            className="p-1 rounded-lg hover:bg-[#A8B4C8]/10 disabled:opacity-50"
          >
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            ...
            disabled={busy}
            className="flex-1 px-4 py-2.5 bg-gradient-to-r from-[#1E3A5F] to-[#A8B4C8] rounded-xl text-white font-semibold hover:opacity-90 transition-opacity flex items-center justify-center gap-2 disabled:opacity-50"
          >
...
function DateField({ label, value, onChange, disabled }) {
  return (
    <div>
      <label className="block text-xs text-[#F5F0E8]/60 mb-1.5 uppercase tracking-wider">
        {label}
      </label>
      <div className="relative flex items-center border border-[#A8B4C8]/20 rounded-xl bg-[#111111]/60">
        <Calendar className="w-4 h-4 text-[#A8B4C8] ml-3 flex-shrink-0 pointer-events-none" />
        <input
          type="date"
          value={value}
          onChange={(e) => onChange(e.target.value)}
          disabled={disabled}
          className="flex-1 px-3 py-2.5 bg-transparent text-[#F5F0E8] focus:outline-none text-sm disabled:opacity-50 cursor-pointer [&::-webkit-calendar-picker-indicator]:opacity-60 [&::-webkit-calendar-picker-indicator]:invert [&::-webkit-calendar-picker-indicator]:cursor-pointer"
        />
      </div>
    </div>
  );
