...
class AuthService:
    ...
    def verify_email_token(self, token: str) -> Optional[User]:
        ...
        if user:
            ...
            user.is_active = True  # Activate account on email verification
            self.db.commit()
            self.db.refresh(user)
    ...
    def request_password_reset_otp(
        ...
        if otp_type == "EMAIL":
            ...
            user = (
                ...
                .filter(UserProfile.phone == identifier)
                .first()
            )
    ...
    def reset_password_with_otp(
        ...
        IMPORTANT: This function does NOT re-verify the OTP code.
        The OTP should already be verified via /api/v1/auth/verify-reset-otp endpoint.
        This function only validates the verification timestamp to prevent abuse.
        ...
        Raises:
            ValueError: If user not found, verification expired, or password validation fails
        """
        from models.user_profile import UserProfile
        from core.redis_client import redis_client

        # STEP 1: Verify that the identifier was recently verified via OTP
        # This prevents abuse while avoiding double OTP verification
        # The verification timestamp is set in /api/v1/auth/verify-reset-otp endpoint
        verification_key = f"password_reset_verified:{otp_type.lower()}:{identifier}"
        verification_data = redis_client.get_cache(verification_key, namespace="")

        if not verification_data:
            # Verification timestamp not found - OTP was not verified or expired
            logger.warning(
                f"Password reset attempt without prior OTP verification. "
                f"Identifier: {identifier}, Type: {otp_type}"
            )
            raise ValueError(
                "Verification expired or not found. Please request OTP and verify again."
            )

        # STEP 2: Validate identifier exists in database (security check)
        # Search by email or phone (phone is in user_profiles)
        user = (
            self.db.query(User)
            .outerjoin(UserProfile, User.id == UserProfile.user_id)
        ...
        if not user:
            logger.warning(
                f"Password reset attempt for non-existent user. "
                f"Identifier: {identifier}, Type: {otp_type}"
            )
        ...
        if not valid:
            logger.warning(
                f"Password reset attempt with weak password for user {user.id}. "
                f"Errors: {errors}"
            )
            raise ValueError("; ".join(errors))
        ...
        try:
            ...
            # Reset security flags on successful password reset
            if user.security:
                user.security.failed_login_attempts = 0
                user.security.locked_until = None
            ...
            # STEP 5: Invalidate all sessions for security (force re-login)
            self.logout_all(user.id)

            # STEP 6: Clean up verification timestamp (one-time use)
            redis_client.delete_cache(verification_key, namespace="")

            # STEP 7: Log the successful password reset
            logger.info(
                f"Password reset successfully via OTP for user {user.id} "
                f"({identifier}, {otp_type})"
            )
            ...
            self.db.rollback()
            logger.error(
                f"Database error during password reset for user {user.id}: {str(e)}",
                exc_info=True
            )
            raise ValueError("Failed to reset password. Please try again.")
