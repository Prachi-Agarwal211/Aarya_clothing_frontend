...
INSERT INTO site_config (key, value, description) VALUES
    ...
    ('intro_video_url_desktop', 'https://pub-7846c786f7154610b57735df47899fa0.r2.dev/Create_a_video_202602141450_ub9p5.mp4', 'URL of the intro video for desktop (16:9)'),
    ('intro_video_url_mobile', 'https://pub-7846c786f7154610b57735df47899fa0.r2.dev/Create_a_video_202602141450_ub9p5.mp4', 'URL of the intro video for mobile (9:16)'),
...
-- Migration: Copy existing intro_video_url to desktop variant (if exists)
-- This handles backward compatibility for existing installations
DO $$
BEGIN
    -- If old intro_video_url exists but new columns don't, migrate it
    IF EXISTS (SELECT 1 FROM site_config WHERE key = 'intro_video_url') 
       AND NOT EXISTS (SELECT 1 FROM site_config WHERE key = 'intro_video_url_desktop') THEN
        ...
        SELECT 'intro_video_url_desktop', value, 'URL of the intro video for desktop (16:9)'
        FROM site_config WHERE key = 'intro_video_url'
        ON CONFLICT (key) DO NOTHING;
        
        -- Copy old URL to mobile (temporary, admin should upload proper mobile video)
        INSERT INTO site_config (key, value, description)
        SELECT 'intro_video_url_mobile', value, 'URL of the intro video for mobile (9:16)'
        FROM site_config WHERE key = 'intro_video_url'
        ON CONFLICT (key) DO NOTHING;
...
-- Add new constraint: Razorpay-only payment method.
DO $$ BEGIN
...
-- Ensure Razorpay payment method is active (Razorpay-only deployment).
...
-- Deactivate/remove other payment methods
UPDATE payment_methods SET is_active = FALSE WHERE name IN ('cashfree', 'cod', 'easebuzz', 'upi', 'bank_transfer', 'wallet');
