  ...
  const columns = [
    {
      ...
      label: (
        <button
          onClick={toggleAll}
          className="text-[#F5F0E8]/60 hover:text-[#F5F0E8] flex items-center justify-center"
        >
          {allSelected ? (
    ...
    },
    {
      ...
      render: (v, row) => (
        <div className="flex flex-col">
          <span className="text-[#D4AF37] font-medium">{v || `ORD-${row.id.toString().padStart(6, '0')}`}</span>
          <span className="text-[10px] text-[#F5F0E8]/40 font-mono uppercase truncate max-w-[80px]" title={row.razorpay_payment_id}>
    ...
    },
    {
      ...
      render: (v, row) => (
        <div className="flex flex-col max-w-[150px]">
          <span className="text-[#F5F0E8] truncate">{v || 'Guest'}</span>
          <span className="text-[10px] text-[#F5F0E8]/40 truncate">{row.customer_email}</span>
        </div>
      ),
    },
    {
      key: 'customer_phone',
      label: 'Phone',
      render: (v) => <span className="text-[#F5F0E8]/70 text-xs">{v || '-'}</span>,
    },
    {
      key: 'item_count',
      label: 'Items',
      sortable: true,
      render: (v) => <span className="text-[#F5F0E8]/80 font-mono">{v || 0}</span>,
    },
    {
      key: 'total_amount',
      label: 'Total (₹)',
    ...
    },
    {
      ...
      sortable: true,
      render: (v) => (
        <span className="text-[#F5F0E8]/60 text-xs">{formatDate(v)}</span>
      ),
    },
  ];
    ...
    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <div>
          ...
          <p className="text-[#F5F0E8]/60 mt-1">Manage and track all customer orders</p>
        </div>
        
        {/* Statistics Pills */}
        <div className="flex gap-2">
          <div className="px-3 py-1 bg-[#1E3A5F]/10 border border-[#A8B4C8]/20 rounded-full">
            <p className="text-[10px] text-[#F5F0E8]/40 uppercase tracking-tighter">Total Orders</p>
            <p className="text-sm font-bold text-[#D4AF37]">{totalOrders}</p>
          </div>
          <div className="px-3 py-1 bg-[#1E3A5F]/10 border border-[#A8B4C8]/20 rounded-full">
            <p className="text-[10px] text-[#F5F0E8]/40 uppercase tracking-tighter">Filtered Revenue</p>
            <p className="text-sm font-bold text-[#D4AF37]">₹{Number(totalRevenue || 0).toLocaleString('en-IN')}</p>
          </div>
        </div>
      </div>
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={onExport}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#1E3A5F]/20 border border-[#A8B4C8]/25 text-[#D4AF37] hover:bg-[#1E3A5F]/35 transition-colors"
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-[#111111]/60 border border-[#A8B4C8]/20 rounded-xl text-[#F5F0E8] placeholder-[#F5F0E8]/40 focus:outline-none focus:border-[#A8B4C8]/40 transition-colors text-sm"
          />
        </div>
        <div className="relative">
          <Filter className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#F5F0E8]/40" />
          <select
            value={status}
            onChange={(e) => onStatusChange(e.target.value)}
            className="w-full md:w-auto pl-10 pr-8 py-2.5 bg-[#111111]/60 border border-[#A8B4C8]/20 rounded-xl text-[#F5F0E8] focus:outline-none focus:border-[#A8B4C8]/40 transition-colors text-sm appearance-none cursor-pointer"
          >
        ...
        {(status || search) && (
          <button
            ...
            className="px-4 py-2.5 text-sm text-[#A8B4C8] hover:text-[#D4AF37] transition-colors"
          >
            Clear filters
          </button>
        )}
          ...
          <button
            ...
            className={`p-4 rounded-xl border transition-all text-left ${
              isActive
                ...
                : `bg-[#111111]/40 ${tone.border} hover:${tone.activeBg}`
            }`}
          >
            <p className={`text-2xl font-bold ${tone.color}`}>{count}</p>
            <p className={`text-xs mt-0.5 font-medium ${isActive ? tone.color : 'text-[#F5F0E8]/60'}`}>
              {status.label}
            </p>
          </button>
        );
...
function BulkActionsBar({ count, busy, onShip, onDeliver, onCancel, onClear }) {
  return (
    <div className="flex flex-wrap items-center gap-2 p-4 bg-[#111111]/40 backdrop-blur-md border border-[#A8B4C8]/15 rounded-2xl">
      <span className="text-sm text-[#F5F0E8]/60 mr-2">{count} selected</span>
      <button
        onClick={onShip}
        disabled={busy}
      ...
      <button
        onClick={onClear}
        className="ml-auto text-xs text-[#F5F0E8]/40 hover:text-[#F5F0E8]/70"
      >
        Clear
      </button>
    </div>
    ...
    <div className="space-y-6 animate-pulse">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="space-y-2">
          ...
          <div className="h-4 bg-[#A8B4C8]/10 rounded w-64" />
        </div>
        <div className="flex gap-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="h-24 bg-[#111111]/40 border border-[#A8B4C8]/15 rounded-xl" />
        ))}
      </div>

      {/* Filters Skeleton */}
      <div className="h-16 bg-[#111111]/40 border border-[#A8B4C8]/15 rounded-2xl" />

      {/* Table Skeleton */}
      <div className="bg-[#111111]/40 border border-[#A8B4C8]/15 rounded-2xl h-[600px]" />
    </div>
  );
}
