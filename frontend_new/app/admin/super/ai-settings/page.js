...
function Section({ icon: Icon, title, description, children, className = '' }) {
  return (
    <div className={`bg-[#180F14] border border-[#A8B4C8]/15 rounded-2xl overflow-hidden ${className}`}>
      <div className="px-6 py-4 border-b border-[#A8B4C8]/10 flex items-start gap-3">
        <div className="w-9 h-9 rounded-xl bg-[#1E3A5F]/20 flex items-center justify-center flex-shrink-0">
          <Icon className="w-4 h-4 text-[#A8B4C8]" />
        </div>
        <div>
          <p className="text-sm font-semibold text-[#F5F0E8]">{title}</p>
          {description && <p className="text-xs text-[#F5F0E8]/40 mt-0.5">{description}</p>}
        </div>
      </div>
  ...
  return (
    <div className={`flex flex-col sm:flex-row sm:items-start gap-3 ${className}`}>
      <div className="sm:w-64 flex-shrink-0">
        <p className="text-sm font-medium text-[#F5F0E8]/80">{label}</p>
        {description && <p className="text-xs text-[#F5F0E8]/35 mt-0.5 leading-relaxed">{description}</p>}
      </div>
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2">
        <div className="flex-1 relative">
          <input
            ...
            className="w-full bg-[#111111] border border-[#A8B4C8]/20 rounded-lg pl-4 pr-12 py-2.5 text-[#F5F0E8] focus:outline-none focus:border-[#A8B4C8] font-mono text-sm"
            placeholder={`Enter ${providerConfig.name} API key`}
          />
          <button
            onClick={() => setShowKey(!showKey)}
            className="absolute right-3 top-1/2 -translate-y-1/2 text-[#A8B4C8]/50 hover:text-[#A8B4C8]"
          >
        ...
        <button
          ...
          className="px-4 py-2 bg-[#1E3A5F] text-white rounded-lg hover:bg-[#A8B4C8] transition-colors"
        >
      ...
      <div className="flex gap-2 items-center">
        <button
          onClick={onTest}
          className="px-4 py-2 bg-[#A8B4C8]/20 text-[#D4AF37] rounded-lg hover:bg-[#A8B4C8]/30 transition-colors flex items-center gap-2 text-sm"
        >
      ...
      {providerConfig.models.map(m => (
        <label
          ...
          className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${
            value === m.value
              ? 'border-[#A8B4C8]/50 bg-[#1E3A5F]/15'
              : 'border-[#A8B4C8]/15 hover:border-[#A8B4C8]/30 bg-[#111111]/50'
          }`}
        >
          <input type="radio" className="hidden" checked={value === m.value} onChange={() => onChange(m.value)} />
          <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
            value === m.value ? 'border-[#A8B4C8]' : 'border-[#A8B4C8]/30'
          }`}>
            {value === m.value && <div className="w-2 h-2 rounded-full bg-[#A8B4C8]" />}
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-2">
              <p className="text-sm text-[#F5F0E8] font-medium">{m.label}</p>
              {m.badge && (
            ...
            <p className="text-xs text-[#F5F0E8]/35 mt-0.5 font-mono">{m.cost}</p>
          </div>
        </label>
      ))}
    </div>
...
function ProviderCard({ providerKey, config, isActive, onActivate, apiKey, onUpdateApiKey, onTestKey, testResult, selectedModel }) {
  ...
  return (
    <div className={`rounded-2xl border-2 overflow-hidden transition-all ${
      isActive 
        ? 'border-[#A8B4C8] bg-[#1E3A5F]/10 shadow-lg shadow-[#A8B4C8]/20' 
        : 'border-[#A8B4C8]/20 bg-[#111111]/50 hover:border-[#A8B4C8]/40'
    }`}>
      {/* Header */}
      <div className="px-6 py-4 border-b border-[#A8B4C8]/10 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: config.bgColor }}>
            <Icon className="w-5 h-5" style={{ color: config.color }} />
          </div>
          <div>
            <h3 className="text-base font-semibold text-[#F5F0E8]">{config.name}</h3>
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
            isActive
              ...
              : 'bg-[#A8B4C8]/20 text-[#D4AF37] hover:bg-[#A8B4C8]/30'
          }`}
        >
      ...
      <div className="p-6 space-y-4">
        {/* API Key */}
        <div>
          <p className="text-xs font-medium text-[#F5F0E8]/60 mb-2 uppercase tracking-wider">API Key</p>
          <ApiKeyInput
        ...
        {isActive && selectedModel && (
          <div>
            <p className="text-xs font-medium text-[#F5F0E8]/60 mb-2 uppercase tracking-wider">Current Model</p>
            <p className="text-sm text-[#D4AF37] font-mono bg-[#111111]/60 px-3 py-2 rounded-lg border border-[#A8B4C8]/10">
              {selectedModel}
            </p>
            <p className="text-xs text-[#F5F0E8]/30 mt-1">Change models in the Model Configuration section below</p>
          </div>
        )}
      </div>
    </div>
...
function Toggle({ value, onChange, label }) {
  ...
  return (
    ...
    >
      {on
        ? <ToggleRight className="w-8 h-8 text-[#A8B4C8]" />
        : <ToggleLeft className="w-8 h-8 text-[#F5F0E8]/30" />
      }
      {label && <span className="text-sm text-[#F5F0E8]/70 group-hover:text-[#F5F0E8] transition-colors">{label}</span>}
    </button>
  );
}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#000000]">
        <div className="text-center">
          ...
          <p className="text-[#F5F0E8]/70">Loading AI Settings...</p>
        </div>
      </div>
    );
  }
      ...
      <Section icon={Cpu} title="Model Configuration" description="Select default AI models for customer and admin interfaces">
        <FieldRow label="Customer-Facing Model" description="Model used for customer chat and assistance">
          <select
            ...
            className="bg-[#111111] border border-[#A8B4C8]/20 rounded-lg px-4 py-2.5 text-[#F5F0E8] focus:outline-none focus:border-[#A8B4C8] w-full max-w-md"
          >
        ...
        <FieldRow label="Max Tokens (Customer)" description="Maximum tokens for customer AI responses">
          <input
            ...
            className="bg-[#111111] border border-[#A8B4C8]/20 rounded-lg px-4 py-2 text-[#F5F0E8] focus:outline-none focus:border-[#A8B4C8] w-32"
          />
        </FieldRow>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FieldRow label="Daily Cost Limit (USD)" description="Maximum AI cost per day">
            <input
              ...
              className="bg-[#111111] border border-[#A8B4C8]/20 rounded-lg px-4 py-2 text-[#F5F0E8] focus:outline-none focus:border-[#A8B4C8] w-full"
              placeholder="1.00"
              step="0.01"
            />
          </FieldRow>
      ...
      <Section icon={Globe} title="Language & Localization" description="Configure language support for AI responses">
        ...
        <FieldRow label="Default Language" description="Default language for AI responses">
          <select
            ...
            className="bg-[#111111] border border-[#A8B4C8]/20 rounded-lg px-4 py-2 text-[#F5F0E8] focus:outline-none focus:border-[#A8B4C8]"
          >
      ...
      <Section icon={Database} title="Advanced Configuration" description="Fine-tune AI behavior and performance">
        <FieldRow label="Conversation History" description="Number of previous messages to keep in context">
          <div className="flex items-center gap-4">
            <div>
              <p className="text-xs text-[#F5F0E8]/40 mb-1">Customer</p>
              <input
                type="number"
                value={settings.CUSTOMER_HISTORY || 6}
                onChange={(e) => updateSetting('CUSTOMER_HISTORY', e.target.value)}
                className="bg-[#111111] border border-[#A8B4C8]/20 rounded-lg px-3 py-1.5 text-[#F5F0E8] focus:outline-none focus:border-[#A8B4C8] w-20"
                min="1"
                max="20"
              />
            </div>
            <div>
              <p className="text-xs text-[#F5F0E8]/40 mb-1">Admin</p>
              <input
                type="number"
