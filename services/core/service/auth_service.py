...
class AuthService:
    ...
    def create_refresh_token(self, user_id: int, role: str, expires_delta: Optional[timedelta] = None) -> str:
    ...
    def create_user(self, user_data) -> Dict[str, Any]:
        ...
        user_with_profile = self.db.query(User).options(joinedload(User.profile)).filter(User.id == user.id).one()
    ...
    def save_verification_token(self, user_id: int, token: str, token_type: str = "email_verification") -> None:
    ...
    def login(self, username: str, password: str, remember_me: bool = False) -> Dict[str, Any]:
        ...
        ).outerjoin(UserProfile, User.id == UserProfile.user_id).filter(
            or_(
        ...
        if user.security and user.security.locked_until and user.security.locked_until > datetime.now(timezone.utc):
            remaining = (user.security.locked_until - datetime.now(timezone.utc)).seconds // 60
        ...
        if not user.email_verified:
            ...
            if settings.is_development:
                raise ValueError("Email verification required. Please check your email to verify your account.")
            raise ValueError("Invalid credentials")
        
        # Verify password
        if not self.verify_password(password, user.hashed_password):
            if user.security:
                user.security.failed_login_attempts = (user.security.failed_login_attempts or 0) + 1
                if user.security.failed_login_attempts >= settings.MAX_LOGIN_ATTEMPTS:
                    user.security.locked_until = datetime.now(timezone.utc) + timedelta(minutes=settings.ACCOUNT_LOCKOUT_MINUTES)
                    self.db.commit()
                    raise ValueError(f"Account locked for {settings.ACCOUNT_LOCKOUT_MINUTES} minutes due to too many failed attempts")
                self.db.commit()
                remaining = settings.MAX_LOGIN_ATTEMPTS - user.security.failed_login_attempts
                raise ValueError("Invalid credentials")
            else:
        ...
        # Generate tokens
        refresh_delta = timedelta(minutes=settings.REFRESH_TOKEN_EXPIRE_MINUTES) if remember_me else None
        access_token = self.create_access_token(user.id, user.role.value)
        refresh_token = self.create_refresh_token(user.id, user.role.value, refresh_delta)
        
        # Create session
        session_id = secrets.token_urlsafe(32)
        session_expiry = settings.SESSION_EXPIRE_MINUTES if remember_me else settings.ACCESS_TOKEN_EXPIRE_MINUTES
        try:
            from core.redis_client import redis_client
            redis_client.create_session(session_id, {"user_id": user.id}, session_expiry)
        except ImportError:
            pass
    ...
    def logout(self, user_id: int, refresh_token: str) -> bool:
        ...
        try:
            ...
            redis_client.blacklist_token(refresh_token, settings.REFRESH_TOKEN_EXPIRE_MINUTES * 60)
    ...
    def verify_reset_token(self, token: str) -> Optional[User]:
        ...
        if not verification or verification.verified_at or verification.expires_at < datetime.now(timezone.utc):
            return None
    ...
    def reset_password(self, token: str, new_password: str) -> bool:
        ...
        """
        verification = self.db.query(EmailVerification).filter(EmailVerification.token == token).first()
        if not verification:
    ...
    def request_password_reset_otp(self, identifier: str, otp_type: str = "EMAIL") -> Dict[str, Any]:
        ...
        from models.user_profile import UserProfile
        user = self.db.query(User).outerjoin(
            UserProfile, User.id == UserProfile.user_id
        ).filter(
            or_(User.email == identifier, UserProfile.phone == identifier)
        ).first()
        ...
        request = OTPSendRequest(
            ...
            phone=user.profile.phone if otp_type == "WHATSAPP" and user.profile else None,
