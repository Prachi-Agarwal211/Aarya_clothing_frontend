  ...
  return (
    <aside
      className={cn(
        ...
        'bg-[#111111]/60 backdrop-blur-xl border-r border-[#A8B4C8]/20',
    ...
    >
      {/* Logo Section */}
      <div className="h-16 flex items-center justify-between px-4 border-b border-[#A8B4C8]/15">
        {!collapsed && (
          <Link
            href="/admin"
          ...
          >
            ...
            <span className="text-sm text-[#F5F0E8]/60">Admin</span>
          </Link>
        )}
        ...
        {isMobile ? (
          <button
            onClick={onClose}
            className="p-2 rounded-lg hover:bg-[#A8B4C8]/10 transition-colors min-w-[44px] min-h-[44px] touch-target"
            aria-label="Close navigation menu"
          >
            <X className="w-5 h-5 text-[#F5F0E8]/70" />
          </button>
        ) : (
          <button
          ...
          >
            {collapsed ? (
              <ChevronRight className="w-5 h-5 text-[#F5F0E8]/70" />
            ) : (
              <ChevronLeft className="w-5 h-5 text-[#F5F0E8]/70" />
            )}
          </button>
        )}
      </div>
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    active
                      ...
                      : isHighlight
                        ? 'bg-gradient-to-r from-[#1E3A5F]/20 to-[#A8B4C8]/15 border border-[#A8B4C8]/25 text-[#D4AF37] hover:from-[#1E3A5F]/30 hover:to-[#A8B4C8]/25'
                        : 'text-[#F5F0E8]/70 hover:bg-[#A8B4C8]/10 hover:text-[#F5F0E8]'
                  )}
                  title={collapsed && !isMobile ? item.name : undefined}
                  aria-current={active ? 'page' : undefined}
                >
                  <Icon className={cn('w-5 h-5 flex-shrink-0', active || isHighlight ? 'text-[#D4AF37]' : '')} />
                  {!collapsed && (
                    <>
                      <span className="font-medium text-sm truncate">{item.name}</span>
                      {isHighlight && (
                        <span className="ml-auto text-xs bg-[#A8B4C8]/30 text-[#D4AF37] px-1.5 py-0.5 rounded-full flex-shrink-0">
                          AI
                        </span>
                      )}
                    </>
      ...
      {/* Bottom Section */}
      <div className="border-t border-[#A8B4C8]/15 p-4">
        {/* Logout */}
        <button
          onClick={() => {
            if (isMobile && onClose) {
          ...
          className={cn(
            ...
            'text-[#F5F0E8]/70 hover:bg-red-500/10 hover:text-red-400',
            'transition-all duration-200',
            'min-h-[44px] touch-target',
