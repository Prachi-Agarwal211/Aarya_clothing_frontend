...
export default function LandingPageConfig() {
  ...
  const handleEditSection = (sectionId) => {
    ...
    if (sectionId === 'video') {
      setEditForm({
        intro_video_url_desktop: siteConfig.intro_video_url_desktop || siteConfig.intro_video_url || '',
        intro_video_url_mobile: siteConfig.intro_video_url_mobile || siteConfig.intro_video_url || '',
    ...
    try {
      ...
      if (editingSection === 'video') {
        await siteConfigApi.updateConfig({
          intro_video_url_desktop: editForm.intro_video_url_desktop,
          intro_video_url_mobile: editForm.intro_video_url_mobile,
  ...
  const handleVideoUpload = async (e, deviceVariant) => {
    ...
    if (file.size > 50 * 1024 * 1024) {
      showToast('Max video size is 50MB', 'error');
      return;
    }

    const tempId = `temp-video-${Date.now()}-${deviceVariant}`;
    
    // Set temporary preview URL
    setEditForm(prev => ({
      ...prev,
      [`intro_video_url_${deviceVariant}`]: URL.createObjectURL(file),
      [`uploading_${deviceVariant}`]: true,
      [`uploadProgress_${deviceVariant}`]: 0
    }));
    ...
    try {
      ...
      setEditForm(prev => ({
        ...prev,
        [`intro_video_url_${deviceVariant}`]: result.video_url,
        [`uploading_${deviceVariant}`]: false,
        [`uploadProgress_${deviceVariant}`]: 100
      }));
      
      showToast(`${deviceVariant === 'desktop' ? 'Desktop' : 'Mobile'} video uploaded`);
    } catch (err) {
      setEditForm(prev => ({
        ...prev,
        [`uploading_${deviceVariant}`]: false
  ...
  const handleDeleteVideo = async (deviceVariant) => {
    const videoUrl = editForm[`intro_video_url_${deviceVariant}`];
    if (!videoUrl || !videoUrl.startsWith('http')) return;
    
    if (!confirm(`Delete ${deviceVariant} video?`)) return;
    
    try {
      await uploadApi.deleteVideo(videoUrl);
      setEditForm(prev => ({
        ...prev,
        [`intro_video_url_${deviceVariant}`]: ''
      }));
      showToast(`${deviceVariant === 'desktop' ? 'Desktop' : 'Mobile'} video deleted`);
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-[#F5F0E8]/60 mt-1 text-sm">Configure homepage sections and manage images</p>
        </div>
        <div className="flex items-center gap-2">
          <a href="/" target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 px-3 py-2 text-sm border border-[#A8B4C8]/20 rounded-xl text-[#F5F0E8]/70 hover:bg-[#A8B4C8]/10 transition-colors">
            <ExternalLink className="w-4 h-4" /> Preview
          </a>
          <button onClick={fetchData} disabled={loading}
            className="p-2 rounded-xl border border-[#A8B4C8]/20 text-[#F5F0E8]/70 hover:bg-[#A8B4C8]/10 transition-colors disabled:opacity-50">
      ...
      {loading && (
        <div className="flex items-center justify-center py-16">
          ...
          <span className="ml-3 text-[#F5F0E8]/60">Loading configuration…</span>
        </div>
      )}
      ...
      {!loading && !error && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            <p className="text-xs font-semibold uppercase tracking-widest text-[#F5F0E8]/40 px-1 mb-3">Sections</p>
            {SECTIONS.map(s => {
              let isActive = true;
              ...
              return (
                <button key={s.id} onClick={() => setActiveSection(s.id)}
                  className={`w-full text-left p-4 rounded-xl border transition-all ${activeSection === s.id
                    ? 'bg-[#1E3A5F]/20 border-[#A8B4C8]/40'
                    : 'bg-[#111111]/40 border-[#A8B4C8]/10 hover:border-[#A8B4C8]/30'}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <span className="text-base">{s.icon}</span>
                      <div>
                        <p className="font-medium text-[#F5F0E8] text-sm">{s.name}</p>
                        <p className="text-xs text-[#F5F0E8]/40">{s.description}</p>
                      </div>
                    </div>
                  </div>
                  <div className="mt-2.5 flex items-center gap-2">
                    <span className={`w-1.5 h-1.5 rounded-full ${isActive ? 'bg-green-400' : 'bg-red-400'}`} />
                    <span className="text-xs text-[#F5F0E8]/40">{isActive ? 'Active' : 'Inactive'}</span>
                    {hasImages && (
                      <span className="text-xs text-[#F5F0E8]/25 ml-auto">{imgCount} image{imgCount !== 1 ? 's' : ''}</span>
                    )}
                  </div>
                </button>
              );
            ...
            <div className="bg-[#111111]/40 backdrop-blur-md border border-[#A8B4C8]/15 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-base font-semibold text-[#D4AF37]" style={{ fontFamily: 'Cinzel, serif' }}>
                    {activeSectionData?.name} Configuration
                  </h2>
                </div>
                {activeSection !== 'collections' && (
                  <button onClick={() => handleEditSection(activeSection)}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-sm border border-[#A8B4C8]/20 rounded-lg text-[#F5F0E8]/70 hover:bg-[#A8B4C8]/10 transition-colors">
              ...
              {activeSection === 'collections' ? (
                <div className="p-4 bg-[#A8B4C8]/5 border border-[#A8B4C8]/10 rounded-xl">
                  <p className="text-sm text-[#F5F0E8]">Collections are automatically pulled from the database.</p>
                  <p className="text-xs text-[#F5F0E8]/60 mt-1">To manage featured collections, use the Collections page.</p>
                </div>
              ) : activeSection === 'video' ? (
                <div className="space-y-4">
                  {/* Desktop Video */}
                  <div className="bg-[#111111]/60 border border-[#A8B4C8]/10 rounded-xl p-4">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-sm font-medium text-[#D4AF37] flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-blue-400" />
                        Desktop Video (16:9)
                      </h3>
                      {editForm.intro_video_url_desktop && (
                        <button
                          onClick={() => handleDeleteVideo('desktop')}
                    ...
                    {editForm.intro_video_url_desktop ? (
                      <div className="relative aspect-video bg-black/40 rounded-lg overflow-hidden">
                        <video
                          src={editForm.intro_video_url_desktop}
                      ...
                      <label className={`flex flex-col items-center justify-center p-8 border-2 border-dashed border-[#A8B4C8]/30 rounded-lg cursor-pointer hover:border-[#A8B4C8]/50 transition-colors ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
                        <Upload className="w-8 h-8 text-[#A8B4C8]/40 mb-3" />
                        <p className="text-sm text-[#F5F0E8]/60 text-center mb-1">Click to upload desktop video</p>
                        <p className="text-xs text-[#F5F0E8]/40">MP4, WebM, or MOV (max 50MB, 16:9 ratio)</p>
                        <input
                          type="file"
                  ...
                  <div className="bg-[#111111]/60 border border-[#A8B4C8]/10 rounded-xl p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {editForm.intro_video_url_mobile && (
                        <button
                          onClick={() => handleDeleteVideo('mobile')}
                          className="p-1.5 text-red-400 hover:bg-red-500/10 rounded-lg transition-colors"
                          title="Delete video"
                        >
                          <Trash2 className="w-4 h-4" />
                    ...
                    {editForm.intro_video_url_mobile ? (
                      <div className="relative aspect-[9/16] max-w-xs mx-auto bg-black/40 rounded-lg overflow-hidden">
                        <video
                          src={editForm.intro_video_url_mobile}
                          className="w-full h-full object-contain"
                          controls
                          playsInline
                        />
                      ...
                      <label className={`flex flex-col items-center justify-center p-8 border-2 border-dashed border-[#A8B4C8]/30 rounded-lg cursor-pointer hover:border-[#A8B4C8]/50 transition-colors ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
                        <Upload className="w-8 h-8 text-[#A8B4C8]/40 mb-3" />
                        <p className="text-sm text-[#F5F0E8]/60 text-center mb-1">Click to upload mobile video</p>
                        <p className="text-xs text-[#F5F0E8]/40">MP4, WebM, or MOV (max 50MB, 9:16 ratio)</p>
                        <input
                          type="file"
                          accept="video/mp4,video/webm,video/quicktime"
                  ...
                  <div className="flex items-center justify-between p-3 bg-[#111111]/60 border border-[#A8B4C8]/10 rounded-xl">
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-[#F5F0E8]">Show intro video</span>
                    </div>
                    <button
                      onClick={() => setEditForm(prev => ({ ...prev, intro_video_enabled: !prev.intro_video_enabled }))}
                      className={`relative w-12 h-6 rounded-full transition-colors ${editForm.intro_video_enabled ? 'bg-green-500/80' : 'bg-[#F5F0E8]/20'}`}
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {Object.entries(activeSectionConfig).filter(([k]) => k !== 'is_active').map(([key, value]) => (
                    <div key={key} className={`bg-[#111111]/60 rounded-xl p-3 border border-[#A8B4C8]/10 ${key === 'story' ? 'sm:col-span-2' : ''}`}>
                      <p className="text-xs text-[#F5F0E8]/40 capitalize mb-1">{key.replace(/_/g, ' ')}</p>
                      <p className={`text-sm text-[#F5F0E8] ${key === 'story' ? '' : 'truncate'}`}>
                ...
                <div className="text-center py-6">
                  <Info className="w-8 h-8 text-[#A8B4C8]/30 mx-auto mb-2" />
                  <p className="text-sm text-[#F5F0E8]/40">No configuration set yet.</p>
                  <button onClick={() => handleEditSection(activeSection)}
                    className="mt-2 text-sm text-[#A8B4C8] hover:text-[#D4AF37] underline transition-colors">
                    Click to configure
                  </button>
                </div>
              )}
              ...
              <div className="bg-[#111111]/40 backdrop-blur-md border border-[#A8B4C8]/15 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-5">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 text-sm bg-[#1E3A5F]/30 border border-[#A8B4C8]/30 rounded-xl text-[#D4AF37] hover:bg-[#1E3A5F]/50 transition-colors font-medium">
                ...
                {activeSectionProducts.length === 0 ? (
                  <div className="py-10 text-center">
                    <Package className="w-10 h-10 text-[#A8B4C8]/20 mx-auto mb-3" />
                    <p className="text-[#F5F0E8]/40 text-sm">No products selected yet.</p>
                    <p className="text-[#F5F0E8]/25 text-xs mt-1">Click &quot;Add Products&quot; to select products for this section.</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {activeSectionProducts.map((lp, idx) => (
                      <div key={lp.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${
                        lp.is_active ? 'bg-[#111111]/60 border-[#A8B4C8]/10' : 'bg-[#111111]/30 border-[#A8B4C8]/5 opacity-50'
                      }`}>
                        <span className="text-xs text-[#F5F0E8]/30 w-5 text-center">{idx + 1}</span>
                        {lp.primary_image ? (
                          ...
                          <div className="w-12 h-12 rounded-lg bg-[#1a0f0a] flex items-center justify-center flex-shrink-0">
                            <Package className="w-5 h-5 text-[#A8B4C8]/30" />
                          </div>
                        )}
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-[#F5F0E8] truncate font-medium">{lp.name}</p>
                          <p className="text-xs text-[#F5F0E8]/50">₹{lp.price}</p>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <button onClick={() => handleToggleLandingProduct(lp)}
                            title={lp.is_active ? 'Hide from landing' : 'Show on landing'}
                            className="p-1.5 rounded-lg hover:bg-[#A8B4C8]/10 transition-colors flex items-center justify-center">
                            {lp.is_active ? <Eye className="w-4 h-4 text-green-400" /> : <EyeOff className="w-4 h-4 text-[#F5F0E8]/30" />}
                          </button>
              ...
              <div className="bg-[#111111]/40 backdrop-blur-md border border-[#A8B4C8]/15 rounded-2xl p-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {activeSection === 'hero' && (
                      <div className="flex items-center gap-1 bg-[#111111]/60 border border-[#A8B4C8]/20 rounded-lg p-1">
                        <button
                    ...
                    <label className={`flex items-center gap-2 px-3 py-1.5 text-sm bg-[#1E3A5F]/30 border border-[#A8B4C8]/30 rounded-xl text-[#D4AF37] hover:bg-[#1E3A5F]/50 transition-colors cursor-pointer font-medium ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
                ...
                {activeSection === 'hero' ? (
                  <div className="space-y-6">
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-[#D4AF37] mb-3 flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-blue-400" />
                        Laptop Images
                        <span className="text-xs text-[#F5F0E8]/40 ml-2">({heroLaptopImages.length})</span>
                      </h3>
                      {heroLaptopImages.length > 0 ? (
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                          {heroLaptopImages.map(image => (
                            <div key={image.id}
                              className="relative aspect-video rounded-xl overflow-hidden bg-[#1E3A5F]/10 group border border-[#A8B4C8]/10">
                              {image.image_url ? (
                                <Image
                                  src={image.image_url}
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                  {image.isUploading ? <Loader2 className="w-8 h-8 text-[#A8B4C8] animate-spin" /> : <ImageIcon className="w-8 h-8 text-[#A8B4C8]/30" />}
                                </div>
                              )}
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-2.5 flex items-center justify-between">
                                  ...
                                  <div className="flex items-center gap-1">
                                    <button
                                      ...
                                      className="p-1.5 bg-[#A8B4C8]/80 rounded-lg text-white hover:bg-[#A8B4C8] transition-colors disabled:opacity-50">
                        ...
                        <div className="py-8 text-center border border-dashed border-[#A8B4C8]/20 rounded-xl">
                          <ImageIcon className="w-8 h-8 text-[#A8B4C8]/20 mx-auto mb-2" />
                          <p className="text-[#F5F0E8]/40 text-sm">No laptop images. Select &quot;Laptop&quot; and upload.</p>
                        </div>
                      )}
                    </div>
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-[#D4AF37] mb-3 flex items-center gap-2">
                        ...
                        Phone Images (9:16)
                        <span className="text-xs text-[#F5F0E8]/40 ml-2">({heroPhoneImages.length})</span>
                      </h3>
                      {heroPhoneImages.length > 0 ? (
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                          {heroPhoneImages.map(image => (
                            <div key={image.id}
                              className="relative aspect-[9/16] rounded-xl overflow-hidden bg-[#1E3A5F]/10 group border border-[#A8B4C8]/10">
                              {image.image_url ? (
                                <Image
                                  src={image.image_url}
                        ...
                        <div className="py-8 text-center border border-dashed border-[#A8B4C8]/20 rounded-xl">
                          <ImageIcon className="w-8 h-8 text-[#A8B4C8]/20 mx-auto mb-2" />
                          <p className="text-[#F5F0E8]/40 text-sm">No phone images. Select &quot;Phone (9:16)&quot; and upload.</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="py-12 text-center">
                    <ImageIcon className="w-12 h-12 text-[#A8B4C8]/20 mx-auto mb-3" />
                    <p className="text-[#F5F0E8]/40 text-sm">No images for this section yet.</p>
                  </div>
                )}
              </div>
            )}
      ...
      {editingSection && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#111111]/95 backdrop-blur-xl border border-[#A8B4C8]/20 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                <span className="text-xl">{SECTIONS.find(s => s.id === editingSection)?.icon}</span>
                <h3 className="text-lg font-semibold text-[#D4AF37]" style={{ fontFamily: 'Cinzel, serif' }}>
                  Edit {SECTIONS.find(s => s.id === editingSection)?.name}
                </h3>
              </div>
              <button onClick={() => setEditingSection(null)}
                className="p-1.5 rounded-lg hover:bg-[#A8B4C8]/10 transition-colors flex items-center justify-center">
                <X className="w-5 h-5 text-[#F5F0E8]/60" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              {editingSection === 'video' ? (
                <>
                  <div>
                    <label className="block text-xs font-medium text-[#F5F0E8]/60 mb-1.5 uppercase tracking-wide">Video URL</label>
                    ...
                    {editForm.intro_video_url && (
                      <div className="mt-3 p-3 bg-[#111111]/60 border border-[#A8B4C8]/15 rounded-xl">
                        <p className="text-xs text-[#F5F0E8]/40 mb-2">Video Preview:</p>
                        <div className="relative aspect-video bg-black/40 rounded-lg overflow-hidden">
                          <video 
                        ...
                        <p className="text-xs text-[#F5F0E8]/40 mt-2 truncate">
                          URL: {editForm.intro_video_url}
                        </p>
                      </div>
                    )}
                  </div>
                  <div className="flex items-center justify-between p-3 bg-[#111111]/60 border border-[#A8B4C8]/15 rounded-xl">
                    <div>
                      <p className="text-sm text-[#F5F0E8]">Show Intro Video</p>
                      <p className="text-xs text-[#F5F0E8]/40">Play video on first visit</p>
                    </div>
                    <button onClick={() => setEditForm(prev => ({ ...prev, intro_video_enabled: !prev.intro_video_enabled }))}
                      className={`relative w-11 h-6 rounded-full transition-colors ${editForm.intro_video_enabled ? 'bg-[#A8B4C8]' : 'bg-[#F5F0E8]/20'}`}>
                ...
                <>
                  {(SECTION_FIELDS[editingSection] || []).map(field => (
                    <div key={field.key}>
                      <label className="block text-xs font-medium text-[#F5F0E8]/60 mb-1.5 uppercase tracking-wide">
                        {field.label}
                      </label>
                      {field.type === 'textarea' ? (
                  ...
                  <div className="flex items-center justify-between p-3 bg-[#111111]/60 border border-[#A8B4C8]/15 rounded-xl">
                    <div>
                      <p className="text-sm text-[#F5F0E8]">Section Active</p>
                      <p className="text-xs text-[#F5F0E8]/40">Show this section on the landing page</p>
                    </div>
                    <button onClick={() => setEditForm(prev => ({ ...prev, is_active: prev.is_active === false ? true : false }))}
                      className={`relative w-11 h-6 rounded-full transition-colors ${editForm.is_active !== false ? 'bg-[#A8B4C8]' : 'bg-[#F5F0E8]/20'}`}>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setEditingSection(null)}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 border border-[#A8B4C8]/20 rounded-xl text-[#F5F0E8]/70 hover:bg-[#A8B4C8]/10 transition-colors text-sm">
                Cancel
              </button>
              <button onClick={handleSaveSection} disabled={saving}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-[#1E3A5F]/30 border border-[#A8B4C8]/30 rounded-xl text-[#D4AF37] hover:bg-[#1E3A5F]/50 transition-colors text-sm font-medium disabled:opacity-50">
      ...
      {showProductPicker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#111111]/95 backdrop-blur-xl border border-[#A8B4C8]/20 rounded-2xl p-6 w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl">
            ...
            <input
              ...
              className="w-full px-4 py-2.5 mb-4 bg-[#111111]/60 border border-[#A8B4C8]/20 rounded-xl text-[#F5F0E8] placeholder:text-[#F5F0E8]/30 focus:outline-none focus:border-[#A8B4C8]/50 text-sm"
            />

            <div className="flex-1 overflow-y-auto space-y-2 pr-1">
              {filteredAllProducts.length === 0 ? (
                <p className="text-center text-[#F5F0E8]/40 py-8 text-sm">No products found</p>
                ...
                return (
                  <button key={p.id} onClick={() => setPickerSelected(prev =>
                    ...
                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ${
                      isSelected ? 'bg-[#A8B4C8] border-[#A8B4C8]' : 'border-[#A8B4C8]/30'
                    }`}>
                    ...
                    {img ? (
                      ...
                      <div className="w-10 h-10 rounded-lg bg-[#1a0f0a] flex items-center justify-center flex-shrink-0">
                        <Package className="w-4 h-4 text-[#A8B4C8]/30" />
                      </div>
                    )}
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-[#F5F0E8] truncate font-medium">{p.name}</p>
                      <p className="text-xs text-[#F5F0E8]/50">₹{p.price}</p>
                    </div>
                  </button>
                );
              })}
            ...
            <div className="flex gap-3 mt-4 pt-4 border-t border-[#A8B4C8]/10">
              <button onClick={() => setShowProductPicker(false)}
                className="flex-1 px-4 py-2.5 border border-[#A8B4C8]/20 rounded-xl text-[#F5F0E8]/70 hover:bg-[#A8B4C8]/10 transition-colors text-sm">
                Cancel
              </button>
      ...
      {editingImage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#111111]/95 backdrop-blur-xl border border-[#A8B4C8]/20 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-[#F5F0E8]/60 mb-1.5 uppercase tracking-wide">Title</label>
              ...
              <div>
                <label className="block text-xs font-medium text-[#F5F0E8]/60 mb-1.5 uppercase tracking-wide">Subtitle</label>
              ...
              <div>
                <label className="block text-xs font-medium text-[#F5F0E8]/60 mb-1.5 uppercase tracking-wide">Link URL</label>
              ...
              <div className="flex items-center justify-between p-3 bg-[#111111]/60 border border-[#A8B4C8]/15 rounded-xl">
                <div>
                  <p className="text-sm text-[#F5F0E8]">Image Active</p>
                  <p className="text-xs text-[#F5F0E8]/40">Show this image on the landing page</p>
                </div>
                <button onClick={() => setImageEditForm(prev => ({ ...prev, is_active: !prev.is_active }))}
                  className={`relative w-11 h-6 rounded-full transition-colors ${imageEditForm.is_active !== false ? 'bg-[#A8B4C8]' : 'bg-[#F5F0E8]/20'}`}>
