...
export function useStockStream(enabled = true) {
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      es.onopen = () => {
        ...
        logger.info('[StockStream] Connected');
      };
      ...
      es.onmessage = (event) => {
        ...
        try {
          ...
          logger.error('[StockStream] Parse error:', err);
        }
      };

      es.onerror = (err) => {
        logger.error('[StockStream] Error:', err);
        setError('Connection error');
        setIsConnected(false);
        
        // Don't auto-reconnect if we've exceeded max attempts
        if (reconnectAttemptRef.current >= MAX_RECONNECT_ATTEMPTS) {
          logger.error('[StockStream] Max reconnect attempts reached. Giving up.');
        ...
        logger.info(`[StockStream] Reconnecting in ${delay}ms (attempt ${reconnectAttemptRef.current}/${MAX_RECONNECT_ATTEMPTS})`);
      ...
      logger.error('[StockStream] Failed to connect:', err);
    }
  }, [getReconnectDelay]);
  ...
  const disconnect = useCallback(() => {
    ...
    if (eventSourceRef.current) {
      ...
      eventSourceRef.current = null;
      setIsConnected(false);
      logger.info('[StockStream] Disconnected');
    }
  }, []);
