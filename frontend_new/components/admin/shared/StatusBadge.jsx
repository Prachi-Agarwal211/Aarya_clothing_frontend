...
export default function StatusBadge({ status, variant = 'auto', size = 'md' }) {
  ...
  return (
    <span
      ...
      role="status"
      aria-label={`Status: ${status?.replace(/_/g, ' ') || 'Unknown'}`}
    >
...
export function PaymentStatusBadge({ status }) {
  ...
  return (
    <span
      ...
      role="status"
      aria-label={`Payment status: ${status?.replace(/_/g, ' ') || 'Pending'}`}
    >
...
export function InventoryStatusBadge({ quantity, threshold = 10 }) {
  ...
  if (quantity <= threshold) {
    return (
      <span
        ...
        role="status"
        aria-label={`Low stock: ${quantity} remaining`}
      >
  ...
  return (
    <span
      ...
      role="status"
      aria-label={`In stock: ${quantity} available`}
    >
...
export function ReturnStatusBadge({ status }) {
  ...
  return (
    <span
      ...
      role="status"
      aria-label={`Return status: ${config.label}`}
    >
      <span className={cn('w-1.5 h-1.5 rounded-full', config.dot)} aria-hidden="true" />
