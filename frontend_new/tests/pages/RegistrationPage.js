...
class RegistrationPage {
  constructor(page) {
    ...
    this.otpDigitInputs = page.locator('input[inputmode="numeric"], input[type="text"][maxlength="1"]');
    this.verifyOtpButton = page.locator('button:has-text("Verify OTP"), button:has-text("Verify")');
    this.resendOtpLink = page.locator('button:has-text("Resend"), a:has-text("Resend")');
  ...
  /**
   * Verify verification method is selected
   */
  ...
  /**
   * Fill OTP digits (for 6-digit OTP inputs)
   */
  async fillOtp(otp) {
    await this.otpDigitInputs.waitFor({ state: 'visible', timeout: 10000 });
    const digits = otp.toString().split('');
    for (let i = 0; i < Math.min(digits.length, 6); i++) {
      await this.otpDigitInputs.nth(i).fill(digits[i]);
    }
    // Auto-submit after filling all digits or click verify button
    if (digits.length === 6) {
      await this.page.waitForTimeout(500); // Wait for auto-submit
    } else {
