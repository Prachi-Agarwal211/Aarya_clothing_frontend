  ...
  return (
    ...
    >
      ...
      <div className="h-16 flex items-center justify-between px-4 border-b border-[#A8B4C8]/15">
        {!collapsed && (
          <Link href="/admin/super" className="flex items-center gap-2">
            ...
            <span className="text-sm text-[#F5F0E8]/60">Super Admin</span>
          </Link>
        )}
        <button
          onClick={onToggle}
          className="p-2 rounded-lg hover:bg-[#A8B4C8]/10 transition-colors"
        >
          {collapsed ? (
      ...
      <nav className="flex-1 py-4 overflow-y-auto">
          ...
          {navigation.map((item) => {
            ...
            return (
              <li key={item.name}>
                <Link
                  href={item.href}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 ${active ? 'bg-[#1E3A5F]/30 text-[#D4AF37] border border-[#A8B4C8]/30' : 'text-[#F5F0E8]/70 hover:bg-[#A8B4C8]/10 hover:text-[#F5F0E8]'}`}
        ...
        {!collapsed && (
          <p className="px-4 pt-4 pb-1 text-xs text-[#F5F0E8]/30 uppercase tracking-widest">Admin</p>
        )}
        <ul className="space-y-1 px-2">
