...
const STATUS_CONFIG = {
  confirmed: { label: 'Order Confirmed', color: 'text-amber-400', bg: 'bg-amber-400/10', icon: Clock },
  shipped:   { label: 'Out for Delivery', color: 'text-cyan-400', bg: 'bg-cyan-400/10', icon: Truck },
  delivered: { label: 'Order Delivered', color: 'text-green-400', bg: 'bg-green-400/10', icon: CheckCircle },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-[#D4AF37]">My Orders</h2>

        {/* Filter */}
        <select
          value={filter}
          onChange={(e) => setFilter(e.target.value)}
          className="px-3 py-2 bg-[#111111]/60 border border-[#A8B4C8]/20 rounded-lg text-[#F5F0E8] focus:outline-none focus:border-[#A8B4C8]/40 text-sm"
        >
      ...
      {loading ? (
        ...
        <div className="p-8 bg-[#111111]/40 backdrop-blur-md border border-[#A8B4C8]/15 rounded-2xl text-center">
          ...
          <p className="text-[#F5F0E8]/50">No orders found</p>
          <Link
            href="/products"
            className="inline-block mt-4 px-6 py-2 bg-gradient-to-r from-[#1E3A5F] to-[#A8B4C8] text-white rounded-xl hover:opacity-90 transition-opacity"
          >
            Start Shopping
          </Link>
        </div>
        ...
        <div className="space-y-4">
          {filteredOrders.map((order) => {
            ...
            return (
              <div
                key={order.id}
                className={`p-4 bg-[#111111]/40 backdrop-blur-md border rounded-2xl transition-all duration-500 ${updatedOrderId === order.id
                  ? 'border-[#D4AF37]/60 ring-1 ring-[#D4AF37]/30'
                  : 'border-[#A8B4C8]/15 hover:border-[#A8B4C8]/30'
                  }`}
              >
                <div className="flex flex-col md:flex-row md:items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span className="font-mono text-[#D4AF37]">{order.order_number}</span>
                  ...
                  <div className="flex items-center gap-2">
                    {order.items?.slice(0, 3).map((item, idx) => (
                      <div
                        key={idx}
                        className="relative w-12 h-14 bg-[#1E3A5F]/10 rounded-lg overflow-hidden flex items-center justify-center flex-shrink-0"
                      >
                    ...
                    {order.items_count > 3 && (
                      <div className="w-12 h-14 bg-[#1E3A5F]/10 rounded-lg flex items-center justify-center text-xs text-[#F5F0E8]/50">
                        +{order.items_count - 3}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="text-right">
                    <p className="text-lg font-semibold text-[#D4AF37]">{formatCurrency(order.total)}</p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    <button
                      ...
                      className="flex items-center gap-1 px-2.5 sm:px-4 py-2 bg-[#A8B4C8]/10 text-[#A8B4C8] hover:bg-[#A8B4C8]/20 hover:text-[#D4AF37] rounded-xl transition-all"
                ...
                {order.status === 'shipped' && (
                  <div className="mt-3 pt-3 border-t border-[#A8B4C8]/10 space-y-1.5">
                    {order.tracking_number && (
                      <div className="flex items-center gap-2 p-2.5 bg-cyan-400/5 border border-cyan-400/20 rounded-xl">
                        ...
                        <div>
                          <p className="text-xs text-[#F5F0E8]/50 uppercase tracking-widest">POD / Tracking Number</p>
                    ...
                    {order.courier_name && (
                      <div className="flex items-center gap-2 p-2.5 bg-purple-400/5 border border-purple-400/20 rounded-xl">
                        ...
                        <div>
                          <p className="text-xs text-[#F5F0E8]/50 uppercase tracking-widest">Courier Service</p>
                ...
                {order.status === 'delivered' && order.delivered_at && (
                  <div className="mt-3 pt-3 border-t border-[#A8B4C8]/10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <p className="text-sm text-[#F5F0E8]/70">
                ...
                {order.items && order.items.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-[#A8B4C8]/10">
                    <button
                      onClick={() => toggleOrderDetails(order.id)}
                      className="flex items-center gap-2 text-sm text-[#A8B4C8] hover:text-[#D4AF37] transition-colors w-full"
                    >
                    ...
                    {expandedOrder === order.id && (
                      <div className="mt-3 space-y-2">
                        {order.items.map((item, idx) => (
                          <div key={idx} className="flex items-start gap-3 p-3 bg-[#111111]/60 rounded-xl">
                            {/* Product Image */}
                            <div className="relative w-16 h-20 bg-[#1E3A5F]/10 rounded-lg overflow-hidden flex-shrink-0">
                              {item.image_url ? (
                                <Image
                                  src={item.image_url}
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="font-medium text-[#D4AF37] text-sm truncate">{item.product_name || 'Product'}</p>
                              <div className="mt-1 flex flex-wrap gap-2 text-xs text-[#F5F0E8]/60">
                                {item.size && <span className="px-1.5 py-0.5 bg-[#1E3A5F]/20 rounded">Size: {item.size}</span>}
                                {item.color && (
                                  <span className="px-1.5 py-0.5 bg-[#1E3A5F]/20 rounded inline-flex items-center gap-1">
                                    {item.color_hex && (
                                ...
                                {item.sku && <span className="px-1.5 py-0.5 bg-[#1E3A5F]/20 rounded font-mono">SKU: {item.sku}</span>}
                              </div>
                              <div className="mt-2 flex items-center gap-3 text-sm">
                                <span className="text-[#F5F0E8]/60">Qty: {item.quantity}</span>
                                {item.unit_price && <span className="text-[#F5F0E8]/60">@ {formatCurrency(item.unit_price)} each</span>}
                                <span className="font-semibold text-[#D4AF37]">{formatCurrency(item.price)}</span>
                              </div>
                            </div>
                          </div>
                        ))}
