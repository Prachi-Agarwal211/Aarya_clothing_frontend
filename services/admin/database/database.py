"""Database connection for admin service with optimized connection pooling."""
...
def init_db():
    ...
    # Also create permissions/staff management tables (separate Base)
    from database.models_permissions import (
        Base as PermissionsBase,
        CustomRole, PermissionPreset, IPRestriction, TimeBasedAccess,
        TwoFactorAuth, StaffSession, AuditLog, StaffTask as PermStaffTask, AIPendingAction,
    )
    try:
        PermissionsBase.metadata.create_all(bind=engine, checkfirst=True)
    except Exception:
